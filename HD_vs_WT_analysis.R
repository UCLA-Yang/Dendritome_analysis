# work directory
setwd("C:/Users/yanyanming77/Desktop/neuro_shape_analysis/Q140_WT/Q140vsWT_Oct-2024_cleaned")
...
############################ read data #######################################
###############################################################################################
data_dir = 'C:/Users/yanyanming77/Desktop/Lmeasure_extraction/Q140_WT_withD1/Lmeasure_extract_all_features'
data_name = 'all_htme_brains_with_registration_1168CPneurons_onlyCP.csv'
stats1 = read.csv(paste0(data_dir, '/', data_name),check.names = FALSE)
colnames(stats1)
dim(stats1) # 1167 neurons

table(stats1$Striatal.Subregion) # 256 CPr, 512 CPi, 397 CPc
table(stats1$Genotype) # 493 WT, 674 Q140
...
# ### Distribution plot of D1 and D2 in brain, by WT and Q140, including all CP and non-CP neurons (for paper only) ###
# df_all = read.csv('C:/Users/yanyanming77/Desktop/Lmeasure_extraction/Q140_WT_withD1/Lmeasure_extract_all_features/all_htme_brains_with_registration_all.csv')
# df_summary = df_all %>%
#   mutate(Brain_ind = paste0(Brain, ' (', Genotype, ')')) %>%
#   group_by(Brain_ind, Type) %>%
#   summarise(Count = n(), .groups = 'drop') %>%
#   group_by(Brain_ind) %>%
#   ungroup() %>%
#   mutate(Brain_ind = factor(Brain_ind, levels = c(sort(unique(Brain_ind[grep("WT", Brain_ind)])),
#                                                   sort(unique(Brain_ind[grep("Q140", Brain_ind)])))))
# total_cnt = df_summary %>%
#   group_by(Brain_ind)%>%
#   summarise(Total = sum(Count), .groups = 'drop')
# df_summary = total_cnt %>%
...
# 
# pdf(paste0("Plots/Type_Distribution_Brain_all_1296_neurons.pdf"), wi = 7, he = 6)
...
order_brains = c('hTME15-1', 'hTME19-2', 'hTME16-1',
                 'hTME15-2', 'hTME18-1', 'hTME20-1', 'hTME24-2')
...
unique_brain_geno = c(paste0(c('hTME15-1', 'hTME19-2', 'hTME16-1'), rep(' (WT)',3)), 
                      paste0(c('hTME15-2', 'hTME18-1', 'hTME20-1', 'hTME24-2'), rep(' (Q140)',4)))
...
replace_substrings = function(v) {
  ...
  v = gsub("hTME24-2\\s*\\(Q140\\)", "12m_Q140#4", v)
...
draw_heatmap = function(data_for_heatmap, data_subset, group_name) {
  ...
  # re-order features and create color map for feature clusters
  feature_reorder = data.frame(clusters[[1]])
  feature_reorder$Feature = rownames(feature_reorder)
  rownames(feature_reorder) = NULL
  feature_reorder = feature_reorder[order(feature_reorder[,1]), ]
  ...
  # add annotation
  annotation_df = data.frame(Cluster = feature_reorder[,1])
  rownames(annotation_df) = feature_reorder$Feature
  
  unique_clusters = unique(feature_reorder[,1])
  num_clusters = length(unique_clusters)
  
  manual_palette = c("#e6194b", "#3cb44b", "#ffe119", "#0082c8", "#f58231",
                    "#911eb4", "#46f0f0", "#f032e6", "#d2f53c", "#fabebe",
                    "#008080", "#e6beff", "#aa6e28", "#fffac8", "#800000",
                    "#aaffc3", "#808000", "#ffd8b1", "#000080", "#808080",
                    "#FFFFFF", "#000000")
  palette_colors = manual_palette[1:num_clusters]
  annotation_color_mapping = setNames(palette_colors, unique_clusters)
...
# Draw heatmaps
pdf(file = "Plots/5_heatmap - normalized_feature_region_pairwise_comparison_by_brain_with_all_pretty-name.pdf", wi = 9, he = 8)
scpp(2.5);
par(mar = c(3,5,3,5));
...
draw_heatmap_d1d2 = function(data_for_heatmap, data_subset, group_name) {
  ...
  # re-order features and create color map for feature clusters
  # order features based on the feature clustering tree
  feature_reorder = data.frame(clusters[[1]])
  feature_reorder$Feature = rownames(feature_reorder)
  rownames(feature_reorder) = NULL
  feature_reorder = feature_reorder[order(feature_reorder[,1]), ]
  ...
  # (new) Sep-18-2024, prettify names of brains
  colnames(data_for_heatmap) = colnames(significance_df) = replace_substrings(colnames(data_for_heatmap))
...
# Draw heatmaps
pdf(file = "Plots/6_heatmap - normalized_feature_region_pairwise_comparison_by_brain_byD1D2_with_all_pretty-name.pdf", wi = 11, he = 9)
scpp(2.5);
par(mar = c(3,5,3,5));
...
colored_heatmap_by_genotype = function(data, cat_features, set_name, cat_name){
  ...
  for(cat in cat_features){
    ...
    for (feature in results$Feature) {
      ...
      # column_means = apply(sub_data[sub_data$Genotype == 'WT', numericCols[[1]]], 2, median, na.rm = TRUE)
...
colored_heatmap_by_type_by_genotype = function(data, cat_features, set_name){
  ...
  for(cat in cat_features){
    ...
    for (feature in results$Feature) {
      ...
      # t_test_result = t.test(formula, data = sub_data, var.equal = TRUE)
      # p_value = t_test_result$p.value
      # coef = t_test_result$estimate['mean in group Q140'] - t_test_result$estimate['mean in group WT']
...
# Save the workbook
saveWorkbook(wb, file = "Results/Radar_CPr.CPi.CPc_pct_change_significance_df_byD1D2.xlsx", overwrite = TRUE)
