# work directory
setwd("C:/Users/yanyanming77/Desktop/neuro_shape_analysis/D1_D2/D1vsD2_boxes/D1D2_box_analysis_Oct-2024_cleaned")
...
############################ read data ########################################################
###############################################################################################
data_dir = 'C:/Users/yanyanming77/Desktop/brain-registration/Daniel_boxes/Daniel_boxes_May-18-2024/outputs_neurons_2024-05-08/TME_morpho_box_May-20-2024_2466neurons.csv'
...
print(n_box_D1) # 196 non-empty boxes
n_box_D2 = length(unique(stats1[stats1$Type == 'D2', 'Box'])) 
print(n_box_D2)# 185 non-empty boxes
...
# Plot: bar plot distribution of box count per MT (filtered DM)
...
colors_nonredundant_feature = c(rep('burlywood1', 6), rep('darkgoldenrod1', 8), rep('coral1', 7))
...
draw_heatmap_distribution_by_cluster = function(data_for_heatmap, data_for_barplot, group_name, bar_name, dendro_cluster_order) {
  ...
  # re-order features based on the feature clustering tree and create color map for feature clusters
...
colored_heatmap_by_type_in_box_kruskal_by_cluster = function(data, data_for_barplot, set_name, bar_name, dendro_cluster_order){
  ...
  # colnames(data_matrix) = colnames(color_matrix)= colnames(text_matrix) = colnames(coef_matrix) = colLabs
  ...
  feature_reorder = data.frame(clusters[[1]])
  feature_reorder$Feature = rownames(feature_reorder)
  # subset the feature_reorder using invariant features
  feature_reorder = feature_reorder[feature_reorder$Feature %in% numericCols, ]
