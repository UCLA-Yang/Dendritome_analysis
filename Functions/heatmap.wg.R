...
##### FIXME: bg.lab.col does not align the color correctly, fix.
...
wgHeatmap = function (
  ...
  mapSeparators = TRUE,
  mapVerticalSeparators = mapSeparators,
  mapHorizontalSeparators = mapSeparators,
  ...
  ... ) 
{
  ...
  # if (is.null(colors)) colors = cm.colors(30);
  ...
  if (!is.null(bg.lab.col))
  {
    ...
    { 
      ...
      sign = 1; y0 = ymin - colColorWidth.bottom.all - mapToColorGap.y*(colColorWidth.bottom.all > 0) #- 0.5*textLabelGap.y.bottom;
  ...
  marginWidth.left = par("mai")[2] / par("pin")[1] * figXrange
  marginWidth.right = par("mai")[4] / par("pin")[1] * figXrange
  ...
  labPos$leftColorAndLabelSetup = list(xr = xr, xl = xl, xtext = xtext, xSpaceForRowLabels = xSpaceForRowLabels, 
           xPosOfRowLabels.relative = xPosOfRowLabels.relative);
  if (nRowColorRows.left > 0)
  {
    labPos$leftColorRectangles = .findFnc(".plotOrderedColorSubplot")(order = rev(1:nShowRows),
  ...
  labPos$rowLabelsPos.left = list(x = xtext, y = labPos$yMid, adj = rowLabelsAdj.left);
  ...
  labPos$rightColorAndLabelSetup = list(xr = xr, xl = xl, xtext = xtext, xSpaceForRowLabels = xSpaceForRowLabels, 
           xPosOfRowLabels.relative = xPosOfRowLabels.relative);
  if (nRowColorRows.right > 0)
  {
    labPos$rightColorRectangles = .findFnc(".plotOrderedColorSubplot")(order = rev(1:nShowRows),
  ...
  labPos$rowLabelsPos.right = list(x = xtext, y = labPos$yMid, adj = rowLabelsAdj.right);
