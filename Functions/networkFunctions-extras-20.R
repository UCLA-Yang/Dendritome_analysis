...
# Changes in 20:
#
#   . Switching heatmapWithDendro to wgHeatmap
#   . Old heatmapWithDendro will still be available under the name heatmapWithDendro1.
#   . Changing interpretation of selectBy in consensusHubs
#   . DESeq2-dependent functions gain otional argument sizeFactorArgs; 
#    . DESeqDataSetFromMatrix.PL now optionally and by default estimates size factors
#   . Bugfix in interleave.gapped that makes the function actually work as intended.
#   . Caution: sizeFactors.general has added a shift to the log-transformation of the expression to prevent problems with
#     log(0). These most likely did not affect any previous analysis since they lead to missing (NA) normalization factors.
...
options(stringsAsFactors = FALSE, deparse.max.lines = 30, max.print = 1000, browserNLdisabled = TRUE);
                       ...
                       ...)
{
  ...
  repX = ceiling(nPoints/length(points.x));
...
# Function legend from package graphics. My change is to call text such that expressions can be plotted properly.

legend.PL = function (x, y = NULL, legend, fill = NULL, col = par("col"), 
...
### PL
    if (inherits(legend, "expression") || inherits(legend[[1]], "expression"))
    {
      ymax = yc * max(1, strheight("M", units = "user", cex = cex)*lineHeightMultiplier/yc)
    } else
### end PL
    ...
    if (plot) {
        ...
        mymapply(text2, xt, yt, labels = legend, MoreArgs = list(adj = adj, cex = cex, 
                           ...
                           ...)
{
  ...
  if (verbose)
  {
    ...
    out = PLBoxplot(x, names = FALSE, plotOutliers = plotOutliers, ...);
  n = length(x);
  box = par("usr");
  xText = c(1:n);
  ...
  if (addScatterplot)
  {
    ...
    if (!is.null(pointLabels))
    {
      ...
      ranks.dn = lapply(x, function(x) if (length(x) > 0) rank(-x) else numeric(0));
...
labeledVioplot = function(x, g = NULL, names = NULL, srt = 45, adj = c(1, 0.5), namesColor = 1, cex.names = 1,
                          yOffsetForXLabels = 0.9,
                          addScatterplot = FALSE,
                          pt.cex = 0.8, pch = 21, pt.col = "blue", pt.bg = "skyblue",
                          randomSeed = 31425, jitter = NULL, jitterAmount = 0.4,
                          pointLabels = NULL, nLabel = 2,
                           ...
                           ...)
{
  ...
  x = as.list(x);
  # Drop all missing
  out = vioplot(x, names = rep("", length(x)), ...);
  n = length(x);
  box = par("usr");
  xText = c(1:n);
  yText = rep(box[3] - yOffsetForXLabels * strheight("M"),  n);
...
labeledBarplot2 = function(x, names, g = NULL, horiz = FALSE, 
                           ...
                           labelOffset = 1.5, 
                           errors.upper = NULL, errors.lower = errors.upper, 
                           errorBarWidth = strwidth("II"), errorBarLwd = 1,
                           lim = NULL, beside = FALSE,
                           ...)
{
  xr = x;
  if (!is.null(errors.upper))
  {
    if (length(errors.upper)!=length(x))
       stop("When 'errors.upper' are given, their length must be the same as length of 'x'.")
    upper = x + errors.upper;
    xr = c(xr, upper);
  } else upper = NULL;
  if (!is.null(errors.lower))
  {
    if (length(errors.lower)!=length(x))
       stop("When 'errors.lower' are given, their length must be the same as length of 'x'.")
    lower = x - errors.lower;
    xr = c(xr, lower);
  } else lower = NULL;

  if (is.null(lim)) lim = range(c(0, xr), na.rm = TRUE);

  if (!is.null(g)) {
    mp = verboseBarplot(x, g, names.arg = rep("", length(names)), horiz = horiz, 
               xlim = if (horiz) lim else NULL, ylim = if (horiz) NULL else lim, ...);
    heights = attr(mp, "height");
    mp.center = mp;
  } else {
    mp = barplot(x, names.arg = rep("", length(names)), horiz= horiz, xlim = if (horiz) lim else NULL, 
                 ylim = if (horiz) NULL else lim, beside = beside, ...);
    heights = x;
    attr(mp, "height") = x;
    attr(mp, "stdErr") = rep(0, length(x));
    if (!is.null(dim(x)) && beside) mp.center = colMeans(mp) else mp.center = mp;
  }

  box = par("usr");

  if (!is.null(errors.upper)) addErrorBars.2sided(x = mp, means = x,
      upper = upper,
      lower= lower,
      width = errorBarWidth,
      lwd = errorBarLwd, horizontal = horiz);

  if (horiz)
  {
    yText = mp.center;
    xText = rep(box[1] - labelOffset * strwidth("W"), length(mp.center));
  } else {
    xText = mp.center;
    yText = rep(box[3] - labelOffset * strheight("M"), length(mp.center));
  }
...
addBarplotSignificanceStars = function(x, y, pValueMat, 
     thresholds = data.frame(p = c(0.05, 0.01, 0.001), symbol = c("*", "**", "***")),
     levelHeight = 2*strheight("M"), 
     bracketHeight = 0.2 * strheight("M"), lineCol = 1, lwd = 1)
{
  plotIndex = rowAndColumnIndex(pValueMat < max(thresholds$p));
  if (nrow(plotIndex)==0) return(NULL);
  plotIndex = plotIndex[ plotIndex[, 1] < plotIndex[, 2], , drop = FALSE]
  pairMax = mapply(function(x1, x2)
  {
    index = which(x >=x1 & x<=x2);
    max(y[index], na.rm = TRUE);
  }, x[plotIndex[, "row"]], x[plotIndex[, "col"]]);
  width = abs(x[plotIndex[, "row"]] - x[plotIndex[, "col"]]);
  plotOrder = order(pairMax, abs(width));
  nPlot = length(plotOrder)
  p.plot = pValueMat[plotIndex[plotOrder, , drop = FALSE]];
  levelHeight = .extend(levelHeight, nPlot);
  height1 = 0;
  plotHeights = numeric(nPlot);
  plot.x1 = x[plotIndex[ plotOrder, "row"]]; 
  plot.x2 = x[plotIndex[ plotOrder, "col"]];
  for (p in 1:nPlot)
  {
     if (height1 < pairMax[plotOrder[p]] + levelHeight[p]) height1 = pairMax[plotOrder[p]] + levelHeight[p];
     x1 = plot.x1[p];
     x2 = plot.x2[p];
     plotHeights[p] = height1;
     lines(c(x1, x1, x2, x2), c(height1-bracketHeight, height1, height1, height1-bracketHeight),
           col = lineCol, lwd = 1, xpd = TRUE)
     text((x1 + x2)/2, height1, pValueStars3(p.plot[p], thresholds = thresholds$p, stars = thresholds$symbol),
          adj = c(0.5, 0), xpd = TRUE)
     height1 = height1 + levelHeight[p];
  }
  list(x1 = plot.x1, x2 = plot.x2, heights = plotHeights);
}
...
thinnedScatterplot = function(x, y, err.x = NULL, err.y = NULL, bins = 100, thinTo = 5, log = "", verbose = FALSE, 
                              ...
                              ...)
{
### FIXME: err.x and err.y are ignored for now.
  if (showProgress)
  ...
  excludePoints = excludePoints[!is.na(excludePoints)];
...
addScatterplotCounts = function(
  ...
  opacity = 0.7)
{
  if (length(cuts.x)==0 || length(cuts.y)==0) 
  {
    warning("addScatterplotCounts: some 'cuts.x' or 'cuts.y' are not present, skipping adding counts.");
    return(NULL);
  }
  ...
  binLims.x2 = c(pmin(cuts.x, rep(box[2], length(cuts.x))), box[2]);
  binLims.y1 = c(box[3], pmax(cuts.y, rep(box[3], length(cuts.y))));  
  binLims.y2 = c(pmin(cuts.y, rep(box[4], length(cuts.y))), box[4]);

  if (concordantOnly) cornerOnly = FALSE;
  ...
  for (bx in 1:nBins.x) for (by in 1:nBins.y) 
    if ((tab[bx, by] > 0 || printZeroCounts) && 
        ...
        (!concordantOnly || (bx == 1 && by==1) || (bx==nBins.x && by==nBins.y && bx==by)))
  {
    ...
    place = try(placeLegend(avoid.x, avoid.y, nPositions = nPositions,
                       ...
                       preferenceBox = c(binLims.x1[bx], binLims.x2[bx],
                                    binLims.y1[by], binLims.y2[by]),
                       limitBox = par("usr"),
                       legend = legend,
...
annotatedScatterplot = function(
  x, y, 
  err.x = NULL,
  err.y = NULL, bins = 100, thinTo = 5, log = "", 
  verbose = TRUE,
  showProgress = FALSE, showPoints = NULL, debug = FALSE,
  showCoeff = FALSE,
  coeff = NULL,
  ...
  addAxisLines = FALSE,
  axisLines.col = "grey",
  addEqualityLine = FALSE,
  equalityLine.col = "grey",
  excludeIndex = NULL,
  ...
  mustShowLabels = NULL,
  mustShow.signifOnly = FALSE,
  cex.sideLabels = 1,
  col.sideLabels = 1,
  col.guideLine = rgb(0.2, 0.2, 0.2, 0.3),
  showPoints.col = NULL,
  showPoints.bg = NULL,
  showPoints.cex = pt.cex,
  showPoints.pch = pch,
  main = "",
  ...
...
)
{
  ...
  x0 = x; y0 = y; np = length(x0);
  col = .extend(col, np);
  bg = .extend(bg, np);
  pch = .extend(pch, np);
  pt.cex = .extend(pt.cex, np);
  if (!is.null(mustShowLabels) & is.null(showPoints)) showPoints = pointLabels %in% mustShowLabels;
  if (showCoeff)
  {
    if (is.null(coeff))
    {
      ...
      {
        x2 = x[-excludeIndex]; y2 = y[-excludeIndex];
      } else {
        x2 = x; y2 = y;
      }
      coef = lm(y2~x2)$coefficients[1, 2];
    }
    main = spaste(main, "coef: ", signif(coef, 2));
    if (verbose) main = spaste(main, ", ");
  }
  scp = thinnedScatterplot(x, y,
                     ...
                     debug = debug, excludePoints = excludeIndex, main = main,
                     ...);
  if (is.null(ab.x))
  {
    if (ab.thresholdType=="p") {
      at.x = -qnorm(ab.threshold/2);
    } else {
  ...
  if (addEqualityLine) abline(a = 0, b = 1, col = equalityLine.col);
  if (nLabel > 0 && length(pointLabels)!=0)
  {
    labInfo = if (labelPointsVersion==2) 
                  labelExtremePoints2(scp$x, scp$y, pointLabels[scp$keptPoints], cex = cex.labels,
                       ratio.pointToChar = 0.33*pt.cex[scp$keptPoints],
                       nLabel = nLabel, nConsider = nConsider, forceLabel = forceLabel,
                       scaleForSelection = scaleForSelection,
                       allowDistantLabels = allowDistantLabels, directions = labelDirections) else
                  labelExtremePoints(scp$x, scp$y, pointLabels[scp$keptPoints], cex = cex.labels,
                       ...
                       scaleForSelection = scaleForSelection, directions = labelDirections);
    if (length(labInfo$label) > 0) {
      avoid2 = fillLabelSpaceWithPoints(labInfo$x, labInfo$y, labInfo$label, cex = cex.labels);
    } else
      avoid2 = list(x = numeric(0), y = numeric(0));
  } else { 
    avoid2 = list(x = numeric(0), y = numeric(0)); 
    labInfo = NULL
  }
  ...
  if (addCounts && addAblines)
  {
    addScatterplotCounts(x = x, y = y,
                         ...
                         concordantOnly = counts.concordantOnly,
                         opacity = counts.opacity);
  }

  # Show additional genes on the side, if requested
  if (mustShow.signifOnly)
  {
    signifLabels = pointLabels[replaceMissing(abs(x) > at.x & abs(y) > at.y, FALSE)];
    mustShowLabels = intersect(mustShowLabels, signifLabels);
  } 
  if (length(mustShowLabels) > 0)
  {
    inLabels = mustShowLabels %in% pointLabels;
    if (!all(inLabels))
      printFlush("annotatedScatterplot: Not all entries in 'mustShowLabels' are in 'pointLabels'.");
    mustShowLabels = mustShowLabels[inLabels]
    addlGenes = setdiff(mustShowLabels, labInfo$label);
    nAddl = length(addlGenes);
    if (nAddl > 0)
    {
      addlIndex = match(tolower(addlGenes), tolower(pointLabels));
      textStep = 1.6*max(strheight(addlGenes)) * cex.sideLabels;
      box = par("usr");
      y0 = box[4]; x0 = box[2] + strwidth("M");
      #yMid = y0-textStep * nAddl/2;
      addlY = seq(from = y0-textStep/2, by = -textStep, length.out = nAddl);
      order = .arrangeLines(x1 = x[addlIndex], x2 = rep(x0, nAddl), y1 = y[addlIndex], y2 = addlY);
      text(rep(x0, nAddl), addlY[order], addlGenes, cex = cex.sideLabels, xpd = TRUE, adj = c(0, 0.5));
      for (i in 1:nAddl)
        lines(c(x[addlIndex[i]], x0), c(y[addlIndex[i]], addlY[order[i]]), col = col.guideLine, xpd = TRUE);
    }
    if (length(mustShowLabels) > 0 & (length(showPoints.col)>0 | !is.null(showPoints.bg)>0))
    {
      nGiven = length(inLabels);
      index = match(mustShowLabels, pointLabels);
      col1 = if (length(showPoints.col) > 0) .extend(showPoints.col, nGiven)[inLabels] else showPoints.col = col[index];
      bg1 = if (length(showPoints.bg) > 0) .extend(showPoints.bg, nGiven)[inLabels] else showPoints.bg = bg[index];
      if (length(showPoints.cex)==np) 
      {
         showPoints.cex = showPoints.cex[index];
      } else {
         if (length(showPoints.cex) > nGiven) stop("Inconsistent input of 'showPoints.cex'.");
         showPoints.cex = .extend(showPoints.cex, nGiven)
      }
      if (length(showPoints.pch)==np) 
      {
         showPoints.pch = showPoints.pch[index];
      } else {
         if (length(showPoints.cex) > nGiven) stop("Inconsistent input of 'showPoints.cex'.");
         showPoints.pch = .extend(showPoints.pch, nGiven)
      }
      points(x[index], y[index], pch = showPoints.pch, col = showPoints.col, bg = showPoints.bg, cex = showPoints.cex);
    }
  }
  invisible(list(scatteplor = scp, labelInfo = labInfo))
}

# Helper function determining whether lines from (x[1], y[1]) to (x[2], y[2]) and (x[3], y[3]) to (x[4], y[4]) cross
.intersect = function(x, y)
{
  if (length(x)!=4 || length(y)!=4) stop("'x' and 'y' must be vectors of length 4, giving z and y coordinates of 4 points.");
  n1x = x[2] - x[1];
  n1y = y[2] - y[1];
  n2x = x[4] - x[3];
  n2y = y[4] - y[3];
  ...
  Da = n2x * my - n2y * mx;
  Db = n1x * my - n1y * mx;
  if (!is.finite(D)) browser()
  if (D==0) return (FALSE);
  a = Da/D; b = Db/D;
  return ( a>=0 && a<=1 && b>=0 && b<=1);
}

.arrangeLines = function(x1, x2, y1, y2, maxIter = 1000)
{
  n = length(x1);
  if (n!=length(x2) || n!=length(y1) || n!=length(y2)) stop("All inputs must be vectors of the same length.");
  if (n==1) return(1);
  intersects = matrix(0, n, n);
  for (i in 1:(n-1)) for (j in (i+1):n)
  {
    intersects[i,j] = intersects[j,i] = .intersect( c(x1[i], x2[i], x1[j], x2[j]), c(y1[i], y2[i], y1[j], y2[j])) + 0;
  }

  order = 1:n;
  iter = 1;
  while (any(intersects==1) && iter <=maxIter)
  {
    r = which(intersects==1)[1] - 1;
    i = floor( r/n ) + 1;
    j = r%%n + 1;
    a = y2[j]
    y2[j] = y2[i];
    ...
    order[j] = order[i];
    order[i] = a;
    if (n > 2) for (ii in setdiff(1:n, c(i, j)))
    {
      intersects[i,ii] = intersects[ii,i] = .intersect( c(x1[i], x2[i], x1[ii], x2[ii]), c(y1[i], y2[i], y1[ii], y2[ii])) + 0;
      intersects[j,ii] = intersects[ii,j] = .intersect( c(x1[j], x2[j], x1[ii], x2[ii]), c(y1[j], y2[j], y1[ii], y2[ii])) + 0;
    }
    intersects[i,j] = intersects[j,i] = 0;
    iter = iter + 1;
  }
  if (iter > maxIter) warning(.immediate = TRUE, ".arrangeLines: exceeded maximum iterations.");
  return(order);
}
  ...
  ...)
{
  ...
  if (is.null(adjColors))
  {
    if (sum(adjx < 0) > 0)
    {
       if (is.null(colors)) colors = blueWhiteRed(100);
       adjColors = numbers2colors(adjacency, signed = TRUE, colors = colors, lim = c(-maxA, maxA));
    } else {
       if (is.null(colors)) colors = blueWhiteRed(100)[50:100];
       adjColors = numbers2colors(adjacency, signed = FALSE, colors = colors, lim = c(0, maxA));
    }
  } else {
    if (!isTRUE(all.equal(dim(adjacency), dim(adjColors))))
      stop("When 'adjColors' is given, its dimensions must equal the dimensions of 'adjacency'.");
  }
  ...
  for (line in startLine:nLines)
  {
    ...
    lines(c(x1,x2),c(y1, y2), lwd = w, col = adjColors[n1, n2]);
  }
...
{
  nm = n-1;
  steps = c(0, rep(c(1:ceiling(nm/2)), rep(2, ceiling(nm/2))) * rep(c(1, -1), ceiling(nm/2)))
  steps = steps[1:n];
                           ...
                           ... ) 
{
  ...
  yLabelsAdj = .extend(yLabelsAdj, nylabels)
  ...
  {
    if (is.null(cex.lab)) cex.lab = 1;
    for (i in yTextLabInd)
    text(xmin - offsetx, labPos$yMid[i], srt = 0, 
         adj = c(yLabelsAdj[i], 0.5), labels = yLabels[i], xpd = TRUE, cex = cex.lab.y, col = colors.lab.y )
  } 
  if (sum(yValidColors)>0)
  {
...
#=====================================================================================================
#
# translateUsingTable. Now capable of handling nested lists and copying attributes
#
#=====================================================================================================

translateUsingTable = function(x, translationTable, keepUntranslated = FALSE, copyAttributes = FALSE)
{
  if (length(x)==0) return(x);
  if (is.atomic(x))
  {
    out = translationTable[ match(x, translationTable[, 1], incomparables = NA), 2]
    if (keepUntranslated) out[is.na(out)] = x[is.na(out)];
    if (copyAttributes) attributes(out) = attributes(x);
  } else { 
    out = lapply(x, translateUsingTable, translationTable, keepUntranslated = keepUntranslated, 
...
{
  t = table(x);
  minn = ceiling(length(x) * minFraction);
                                      ...
                                      levelSep = ".", returnMatrix = FALSE, check.names= TRUE)
{
  ...
  names1 = spaste(varName, nameSep, sapply(covarLevels, paste, collapse = levelSep));
  if (check.names) names1 = make.names(names1);
  out = setNames(as.data.frame(out), names1);
  if (returnMatrix) as.matrix(out) else out;
}
...
restrictVariableByCovariateCombinations = function(var, covar, varName, covarName,
                                      combinations,
                                      combinationNames = names(combinations),
                                      nameSep = spaste(".in.", covarName, if (covarName=="") "" else "."),
                                      levelSep = ".", returnMatrix = FALSE, check.names= TRUE)
{
  n = length(combinations);
  if (n<1) stop("No 'covarLevels' given.");
  allCovarLevels = unique(covar);
  lapply(combinations, function(cl) if (!all(cl %in% allCovarLevels))
    warning(immediate. = TRUE, "Some entries in 'combinations' are not present in 'covar'."));
  out = list();
  ...
  names1 = spaste(varName, nameSep, sapply(combinationNames, paste, collapse = levelSep));
  if (check.names) names1 = make.names(names1);
  out = setNames(as.data.frame(out), names1);
  if (returnMatrix) as.matrix(out) else out;
}
...
#### FIXME: this function should be re-written as a wrapper for heatmapWithDendro.

corHeatmapWithDendro.file = function(
  corMat,
  file, 
  device = "pdf",
  ...
  widthPerCell = 0.5, heightPerCell = 0.2,
  marginW = marginWidth(labels),
  width = baseWidth + dendroWidth + marginW$inch + ncol(corMat) * widthPerCell, 
  height = baseHeight + 0.8 * marginW$inch + nrow(corMat) * heightPerCell,
  mar.title= 2,
  mar.main = c(0.8 * marginW$lines, marginW$lines+0.2, mar.title, 1),
  ...)
{
  match.fun(device)(file = file, width = width, height = height);
  on.exit(dev.off());
  corHeatmapWithDendro(corMat,
     mar.main = mar.main, 
     xLabels = colLabels, yLabels = rowLabels,
     dendroWidth = dendroWidth/width,
     ...);
}
...
#### FIXME: this function should be re-written as a wrapper to heatmapWithDendro.
heatmapWithDendro1 = function(
  mat,
  plotRowTree = TRUE,
  plotColTree = TRUE,
  rowTree = NULL,
...
if (FALSE)
{
  ...
  col = cbind(labels2colors(x), numbers2colors(x), labels2colors(x+1), numbers2colors(rnorm(length(x))));
  col = cbind(col, col);

  plotDendroAndColors(tree, col);
}


#===========================================================================================================
#
# "New" heatmap with dendro, based on wgHeatmap
#
#===========================================================================================================
...
heatmapWithDendro = function(
  ...
  rowTree = NULL, 
  colTree = NULL, 
  abHeight.colTree = NULL,
  abHeight.rowTree = NULL,
  abLine.colTree.col = 1,
  abLine.rowTree.col = 1,
  abLine.colTree.lty = 1,
  abLine.rowTree.lty = 1,
  abLine.colTree.lwd = 1,
  abLine.rowTree.lwd = 1,
  maxMerge = 18, 
  setLayout = TRUE, 
  dendroWidth = 0.25, 
  dendroHeight = 0.25, 
  ...
  main = "", ...)
{
  #printFlush("FYI: Old heatmapWithDendro is available under the name heatmapWithDendro1.");
  nr = nrow(mat);
  nc = ncol(mat);
  if (plotRowTree)
  {
  ...
  if (plotRowTree)
  {
    ...
    plotDendrogram(rowTree.rev, labels = FALSE, hang = -1, horiz = TRUE, reverseDirection = FALSE,
                   adjustRange = TRUE);
    if (length(abHeight.rowTree) > 0)
      abline(v = abHeight.rowTree, col = abLine.rowTree.col, lty = abLine.rowTree.lty, lwd = abLine.rowTree.lwd);
  }
  if (plotColTree)
  {
    par(mar = marColDendro);
    plotDendrogram(colTree, labels = FALSE, hang = -1, horiz = FALSE, reverseDirection = FALSE,
                   adjustRange = TRUE, shift.last = colTree.shift, main = main);
    if (length(abHeight.colTree) > 0)
      abline(h = abHeight.colTree, col = abLine.colTree.col, lty = abLine.colTree.lty, lwd = abLine.colTree.lwd);
  }
  par(mar = mar.main);
  if (showText)
  {
                           ...
                           invertGroupMembership = FALSE)
{
  ...
  for (gl in 1:nLevels)
  {
    indicator = as.numeric(!is.na(g) & (if (invertGroupMembership) g!=gLevels[gl] else g==gLevels[gl]));
...
colMeansByGroup.simple = function(x, g = NULL, assignNames = TRUE, simplify = TRUE, means = TRUE)
## No weights or missing values here
## If means is FALSE, return sums
{
  x = as.matrix(x);
  if (is.null(g)) g = rep(1, nrow(x));
  g = as.factor(g);
  ...
  for (gl in 1:nLevels)
  {
    indicator = which(g==gLevels[gl]);
    if (means) n = length(indicator) else n = 1
    groupSum[gl, ] = colSums(x[indicator, , drop = FALSE])/n;
  }
  groupSum
}

colMeansByGroup = function(x, g = NULL, w = NULL, assignNames = TRUE, simplify = TRUE, means = TRUE)
## If means is FALSE, return sums
{
  x = as.matrix(x);
                               ...
                               getDifference = FALSE)
{
  ...
  if (!is.null(colStats)) nCols = ncol(colStats$means) else nCols = ncol(x);
  ...
  dif = groupMean[g.num, ] - groupMean[g.denom, ];
  t = dif/denom;
  t[!is.finite(t)] = NA;
  ...
  out = cbind(statistic = t, parameter = df, p.value = p, Z = Z);
  if (getDifference) out = cbind(out, difference = dif)
  out;
}

TTestStatNames = function(getDifference)
{
  c("statistic", "parameter", "p.value", "Z", if (getDifference) "difference" else NULL);
}
...
matrixKruskalTest.singleG = function(x, g, getSign, getAreaUnderROC, r = NULL, getFDR = FALSE)
{
  gFinite = is.finite(g);
  nGLevels = length(unique(g[gFinite]));
  nCols = ncol(x);
  if (!any(gFinite) || nGLevels < 2)
  {
    out = matrix(NA, nCols, 4 + getSign + getAreaUnderROC + getFDR);
    colnames(out) = KTstatNames(getSign, getAreaUnderROC, getFDR)
    return(out);
  }
  ...
  if (is.null(r)) r = colRanks(as.matrix(x), ties.method = "average", preserveShape = TRUE);
  ...
  {
    warning("Sign can only be determined for 2-group comparisons. Sign will be invalid.");
    #getSign = FALSE;
  ...
  Z = -qnorm(p/2) * sign;
  out = cbind(statistic = K, parameter = df, p.value = p)
  if (getFDR) out = cbind(out, FDR = p.adjust(p, method = "fdr"));
  out = cbind(out, Z = Z);
...
matrixTest = function(x, g, testFnc, statNames, flatOutput = FALSE, testNameSep = ".for.", 
                      testNames = colnames(g), ...)
{
  ...
  if (length(d) > 1)
  {
     ...
     if (flatOutput)
     {
       lst = lapply(1:d[2], function(i)
       {
         x1 = out[, , i];
         rownames(x1) = colnames(x);
         setColnames(x1, spaste(statNames, testNameSep, testNames[i]))
       });
...
KTstatNames = function(getSign, getAreaUnderROC, getFDR)
{
  statNames = c("statistic", "parameter", "p.value", if (getFDR) "FDR" else character(0),  "Z");
                       ...
                       getDifference = FALSE)
{
  ...
  matrixTest(x, g, testFnc = matrixTTest.singleG, statNames = TTestStatNames(getDifference),
             reverseGroups = reverseGroups, alternative = alternative, var.equal = var.equal,
             getDifference = getDifference);
}

matrixKruskalTest = function(x, g, getSign = FALSE, getAreaUnderROC = FALSE, getFDR = FALSE,
                        flatOutput = FALSE, testNameSep = ".for.", testNames = colnames(g))
{
  if (!any(is.na(g)) && all(is.finite(x)))
  {
    printFlush("matrixKruskalTest: pre-ranking x.");
    r = colRanks(as.matrix(x), ties.method = "average", preserveShape = TRUE);
  } else r = NULL;
  matrixTest(x, g, testFnc = matrixKruskalTest.singleG, 
             statNames = KTstatNames(getSign, getAreaUnderROC, getFDR), 
             getSign = getSign, getAreaUnderROC = getAreaUnderROC, getFDR = getFDR,
             r = r, flatOutput = flatOutput, testNameSep = testNameSep, testNames = testNames);
}
...
metaAnalysis.effects = function(effects, stdErrors = NULL, pValues = NULL,
                               Zs = NULL,
                               meanName = "mean")
{
  ...
  if (is.null(stdErrors))
  {
    if (is.null(Zs))
    {
      if (is.null(pValues)) stop("One of 'stdErrors', 'Zs' or 'pValues' must be given.");
      pValues = as.matrix(pValues)
      if (!isTRUE(all.equal(dim(pValues), dim(effects)))) stop("Dimensions of 'effects' and 'pValues' must be the same.");
      Zs = sign(effects) * qnorm(pValues/2, lower.tail = FALSE)
    } else {
      if (!isTRUE(all.equal(dim(Zs), dim(effects)))) stop("Dimensions of 'effects' and 'Zs' must be the same.");
    }
    stdErrors = effects/Zs;
    if (any(replaceMissing(stdErrors < 0))) 
       stop("Some standard erros calculated from Zs are negative which indicates inconsistent input.");
  } else {
    if (!isTRUE(all.equal(dim(stdErrors), dim(effects)))) stop("Dimensions of 'effects' and 'stdErrors' must be the same.");
  }

  stdErrors = as.matrix(stdErrors);
  weightMat = 1/stdErrors^2;
  metaEffects = rowSums(effects*weightMat, na.rm = TRUE)/ rowSums(weightMat, na.rm = TRUE);
  metaStdErrors = 1/rowSums(weightMat, na.rm = TRUE);
  metaZ = metaEffects/metaStdErrors;
  metaP = 2*pnorm(abs(metaZ), lower.tail = FALSE);
  metaFDR = p.adjust(metaP, method = "fdr");

  out = data.frame(XXX = metaEffects,
                   SE = metaStdErrors, 
                   Z =  metaZ,
                   p = metaP,
                   FDR = metaFDR);
  colnames(out) = sub("XXX", meanName, colnames(out));
  rownames(out) = rownames(effects);
  out;
}
...
#=================================================================================================
#
# rbind disregarding column names and dropping length-zero arguments
#
#=================================================================================================
...
rbind.ncn = function(...)
{
  ...
  ncols = sapply(args, function(x) if (is.null(dim(x))) length(x) else ncol(x));
  if (!all(ncols==ncols[1]))
    stop("Numbers of columns or lengths of arguments are not consistent.");

  colnames1 = colnames(args[[1]]);
  args2 = lapply(args, function(x) if (!is.null(dim(x))) setColnames(x, colnames1) else setNames(x, colnames1));
  setColnames(do.call(base::rbind, args2), colnames1);
}
...
addErrorBars.2sided = function (x, means, upper, lower=NULL, width = strwidth("II"), horizontal = FALSE, ...) 
{
    ...
    if (horizontal)
    {
      for (i in 1:length(means)) {
          segments(means[i], x[i], ERR1[i], x[i], ...)
          segments(ERR1[i], x[i] - width/2, ERR1[i], x[i] + width/2, ...)
          if (!is.null(ERR2))
          {
            segments(means[i], x[i], ERR2[i], x[i], ...)
            segments(ERR2[i], x[i] - width/2, ERR2[i], x[i] + width/2, ...)
          }
      }
    } else 
      for (i in 1:length(means)) {
...
multiPlot = function( x = NULL, y = NULL, data = NULL,
                      ...
                      shiftX = 0,  ## this argument applies when x is repeated; a shiftX multipled nby index will be added
                      type = "p",
                      xlim = NULL, ylim = NULL, 
                      ...
                      ...)
{
  ...
  {
    x1 = x[[1]];
    for (c in 1:ny) x[[c]] = x1 + shiftX * ( (c-ny/2-0.5)/(ny-1))
    nx = length(x);
  }
                            ...
                            prettifyList = NULL, ...)
{
  ...
  lst = data.frame(long = legend.long, standard = legend.std, short = legend.short)
  if(length(prettifyList) > 0)
  {
     lst = as.data.frame(lapply(lst, prettifyStrings, prettifyList))
  }
  finalLegend = lst$long;
  tooLong = nchar(finalLegend) > maxLegendLength
  finalLegend[tooLong] = lst$standard[tooLong];
  tooLong = nchar(finalLegend) > maxLegendLength
  finalLegend[tooLong] = lst$short[tooLong];

  list(colors = colors, legend = finalLegend, allLegends = lst)
}
...
dist = function(x, method = "euclidean", diag = FALSE, upper = FALSE, p = 2, large = FALSE)
{
  if (!large) x = as.matrix(x);
  if (method!="euclidean") 
  ...
  if (any(is.na(x)))
  {
    ...
    square = outer(diagonal, diagonal, `+`) -2*square;
    square[square<0] = 0;
    if (large) print(gc())
    out = as.dist(sqrt(square), diag = diag, upper = upper);
    if (large) print(gc())
...
# This version returns a distance matrix and works only for Euclidean distance.
mdist = function(x)
{
  if (any(is.na(x)))
  {
     ...
     square1 = (xf*xf) %*% t(fin);
     fSquare = fin %*% t(fin);
     sqrt( (square1 + t(square1) - 2*square)/ (fSquare/max(fSquare)));
  } else {
    #printFlush("using fast Euclidean distance calculation...");
    square = x %*% t(x);
    diagonal = diag(square);
...
dist2 = function(x, y, setDimnames = TRUE)
{
  xsq = rowSums(x^2)
  ysq = rowSums(y^2)
  xy = 2 * x %*% t(y);
  out = suppressWarnings(sqrt(outer(xsq, ysq, `+`) - xy));
  out[is.nan(out)] = 0;
  if (setDimnames)
  {
...
consensusHubs = function(consKME, 
                         ...
                         returnColumns = "ID",
                         simpleOutput = TRUE,
                         pickOneNonMissing = TRUE,
                         dropMissing = TRUE)
{
  selectCols = grep(spaste(selectBy, "[0-9.]+$"), colnames(consKME));  ## The module lables may be decimal...
  moduleLevels = gsub(selectBy, "", colnames(consKME)[selectCols]);
  numLevels = suppressWarnings(as.numeric(moduleLevels))
  if (all(is.finite(numLevels))) moduleLevels = sort(numLevels);
  ret.mat = consKME[, match(returnColumns, colnames(consKME)), drop = FALSE];
  retMissing = rowSums(!is.na(as.matrix(ret.mat)))==0;
  ...
  orders = mymapply(function(mm, st)
    {
      st[!mm] = NA;
      if (dropMissing) st[retMissing] = NA;  
  ...
  if (ncol(ret.mat)==1 && simpleOutput)
  {
    ...
    colnames(hubs) = moduleLevels;
  } else {
    hubs = lapply(orders, function(i, data, n) data[i[1:n], ], ret.mat, n=nHubs);
    if (pickOneNonMissing)
    {
      hubs = lapply(hubs, function(mat)
      {
        out = mat[, 1]; ind = 2;
        while (any(is.na(out)) && ind <= ncol(mat))
        {
          out[is.na(out)] = mat[is.na(out), ind];
          ind = ind + 1;
        }
        out;
      })
      hubs = setColnames(do.call(cbind, hubs), moduleLevels);
    } else
      names(hubs) = moduleLevels;
  }
  hubs;
}
                                   ...
                                   ...)
{
  ...
  par(lheight = 0.85);
  plotOrderedColors(order, traitColors, rowLabels = traitNames, startAt = 0.5, separatorLine.col = NA, 
                     ...);
  out
}
...
splitData = function(data, split, nameBase = "", includeInAllSets = NULL, drop = FALSE)
{
  groups = levels(factor(split));
  nGroups = length(groups);

  if (length(includeInAllSets) > 0 && is.logical(includeInAllSets)) includeInAllSets = which(includeInAllSets);
  splitData = list();
  for (g in 1:nGroups)
  {
     splitData[[g]] = list(data = data[union(which(split==groups[g]), includeInAllSets), , drop = drop]);
  }
                             ...
                             removePatternBeforeMatch = "", fixed = FALSE)
{
  ...
  {
     colnames.match = lapply(colnames1, sub, pattern = removePatternBeforeMatch, replacement = "", fixed = fixed);
  } else
...
pValueStars3 = function(p, thresholds = c(0.05, 0.01, 0.001), stars = c("*", "**", "***"))
{
  out = rep("", length(p));
  for (i in 1:length(p))
  {
    t1 = which(thresholds >= p[i]);
    if (length(t1) > 0)
    {
      ind = max(t1);
      out[i] = stars[ind];
                        ...
                        ...)
{
  ...
  if (addGrid) addGrid(horiz = FALSE, vert = TRUE, linesPerTick = linesPerTick, lty = grid.lty, col = grid.col);
    ...
    ...)
{
  ...
  if (is.null(rowsPerPage))
  {
    nPages.rows = ceiling(n/maxRowsPerPage);
    rowsPerPage = allocateJobs(n, nPages.rows);
  } else
    nPages.rows = length(rowsPerPage);
...
alignedBarplot = function(x, space = 0.2, add = FALSE, 
      ylim = if (!horizontal) .range2(x) else NULL,
      xlim = if (horizontal) .range2(x) else NULL, 
      col = 0, border = 1, xlab = "", ylab = "",
      plot = TRUE, horizontal = FALSE, ...)
{
  .range2 = function(x) c ( min(0, min(x, na.rm = TRUE)), max(0, max(x, na.rm = TRUE)))
  n = length(x);
  if (!add) plot(c(0:1), c(0:1), type = "n", xaxt = "n", ylim = ylim, xlim = xlim, xlab = xlab, ylab = ylab, ...);
  if (n==0) return(NULL);
  box = par("usr");
  if (!horizontal)
  {
    step = (box[2] - box[1])/n;
    xMid = box[1] + ( (1:n)- 0.5) * step;
    w = step*(1-space);
    xLeft = xMid - w/2;
    xRight = xMid + w/2;
    if (plot) rect(xLeft, 0, xRight, x, col = col, border = border);
    out = list(xLeft = xLeft, xRight = xRight, xMid = xMid, box = box, y = x);
  } else {
    step = (box[4] - box[3])/n
    yMid = box[3] + ( (1:n)- 0.5) * step;
    w = step*(1-space);
    yBottom = yMid - w/2;
    yTop = yMid + w/2;
    if (plot) rect(0, yBottom, x, yTop, col = col, border = border)
    out = list(yBottom = yBottom, yTop = yTop, yMid = yMid, box = box, x = x);
  }
  invisible(out);
}
   ...
   labelPrefix = "M")
{
   ...
   for (m in 1:nModules)
   {
     ...
     description1 = multiSub(spaste("%", colnames(moduleInfo)), as.character(unlist(moduleInfo[m, ])),
                             descriptionPattern);
     if (!is.null(shortNamePattern)) {
       shortName1 = multiSub(spaste("%", colnames(moduleInfo)), as.character(unlist(moduleInfo[m, ])),
                             shortNamePattern);
...
deseqBicovWeightsFromMatrix = function( 
   ...
   ...)
{
  object = DESeqDataSetFromMatrix.PL(t(counts), colData = pheno, design = design, sizeFactorArgs = sizeFactorArgs);
  #if (!is.null(sizeFactors)) sizeFactors(object) = sizeFactors;
...
replaceOutlierCounts = function(object, blind = FALSE,
                                maxPOutliers = 0.05,
                                useApproximateVST = FALSE,
                                approximateVST.minShift = 1,
                                approximateVST.maxShift = 500,
                                ...
                                )
{
  ...
  if (returnDESeqDataSet) {
     ...
     DESeqDataSetFromMatrix.PL(t(replacedCounts), design = design(object), 
                         colData = colData(object), sizeFactorArgs = attr(object, "sizeFactorArgs"));
  } else
     replacedCounts;
}
   ...
   ...)  # The dots are currently not used, only there for compatibility with other functions
{
  expr.ds = DESeqDataSetFromMatrix.PL(countData = t(counts),
                     colData = pheno,
                     design = as.formula(design), sizeFactorArgs = sizeFactorArgs, estimateSizeFactors = TRUE);

  # It should not be necessary to run full DESeq to get variance stabilized data (?)
  #if (!useApproximateVST) 
...
)
{
  ds= DESeqDataSetFromMatrix.PL(
                     countData = t(counts),
                     colData = pheno,
                     design = as.formula(design), sizeFactorArgs = sizeFactorArgs,
                     estimateSizeFactors = TRUE);
  if (useApproximateVST)
  {
...
DESeqDataSetFromMatrix.PL = function(countData, colData, sizeFactorArgs = list(type = "ratio", locfunc = stats::median), 
    estimateSizeFactors = TRUE, ...)
{
  if (ncol(countData)!=nrow(colData)) browser()
  rownames(colData) = colnames(countData);
  out = DESeqDataSetFromMatrix(countData, colData, ...);
  if (estimateSizeFactors) out = do.call(DESeq2:::estimateSizeFactors.DESeqDataSet, c(list(object = out), sizeFactorArgs));
  attr(out, "sizeFactorArgs") = sizeFactorArgs;
  out;
}
...
replaceMissing = function(x, replaceWith = NULL)
{
  if (!is.atomic(x)) {
    lapply(x, replaceMissing, replaceWith = replaceWith);
  } else {
    if (length(replaceWith)==0)
    {
      if (is.logical(x)) {
        replaceWith = FALSE
...
rearrangeInternalCollections = function(collections,
   ...
   separateSCHttSets = FALSE,
    SCHttTags = c("Genes DE in selected cell types of HD patients and models, Lee 2020"))
{
  ...
  if (separateSCHttSets)
  {
    scHtt = subsetCollection(pool, tags = SCHttTags)
    pool = subsetCollection(pool, tags = SCHttTags, invertSearch = TRUE)
  } else
    scHtt = NULL;
  ...
  if (separateSCHttSets) rearranged$DEForHttMutationInSortedCells = scHtt;
...
setRownames = function(x, newRownames)
{
                                ...
                                sizeFactorArgs = list(type = "ratio", locfunc = stats::median))
{
  ...
  while (p <= nPermutations && errCount <= maxContiguousErrors)
  {
    ...
    res = try({
      dss = DESeq(DESeqDataSetFromMatrix.PL(t(counts), colData = permCD, design = design, sizeFactorArgs = sizeFactorArgs), ...);
                     ...
                     keepTempDir = FALSE)
{
  ...
  nBlocks = ceiling(nVars/blockSize);
       ...
       regularizationShift = 0)
{
  ...
  nKMEBlocks = ceiling(nProbes/30000);
...
autoLayout = function(nPlots, wide = FALSE, maxPerPage = 10)
{
  ...
  if (nPlots < 6)
  {
    if (wide || nPlots < 4)
    {
       ...
       return(c(2, ceiling(nPlots/2)));
  };

  c(2, ceiling(nPlots/2));
}
...
### Potentially confusing: significantOrTop will return the index when IDs are NULL or when returnIDs is FALSE. I.e.,
### returnIDs is ignored if IDs are NULL.

significantOrTop = function(IDs = NULL, backupIDs = NULL, Z, p=NULL, pThreshold = 0.05, 
                            p.adjusted, pAdjThreshold = 0.05, zThreshold = 0, nTop = 100,
                            extraStat = NULL,
                            extraThreshold = NULL,
                            returnIDs = !is.null(IDs), warn = TRUE)
{
  if (!is.null(IDs))
  {
...
{
  if (length(at.y) > 0 && is.finite(at.y))
  {
    abline(h=at.y, col = col, ...);
    abline(h=-at.y, col = col, ...);
  }
  if (length(at.x) > 0 && is.finite(at.x))
  {
...
# if all Zs turn out to be significant, return the minimum absolute value shifted by a fraction of the range
ablinePositionFromFDR = function(Z, fdr = fdrFromZ(Z,p, alternative), threshold = 0.1, p = NULL, 
                          alternative = c("two.sided", "greater", "less"),
                          defaultShiftFraction = 0.02)
{
  ...
  if (length(signif)>0)
  {
    ...
    if (length(nonSignif) > 0) 
    {
       ...
       range1 = range(Z, na.rm = TRUE)
       sh = defaultShiftFraction * (range1[2] - range1[1]);
       out = max(0, abs(Z[i1])-sh);
    }
    out;
  } else NA;
}
...
grepSingleOrError = function(pattern, x, ..., stopOnMultiple = TRUE, tryExactMatch = FALSE)
{
  if (tryExactMatch)
  {
    ind = match(pattern, x);
    if (!is.na(ind)) return(ind);
  }
  g = grep(pattern, x, ...)
...
{
  keep = replaceMissing(from!="")
  if (!is.null(df)) names(df) = multiGSub(from[keep], to[keep], names(df), fixed = fixed);
  df;
}
...
{
  keep = which(!duplicated(prettifyList[[1]]) & replaceMissing(prettifyList[[1]]!=""))
  as.data.frame(lapply(prettifyList, `[`, keep));
}

mergePrettifyLists = function(..., order = TRUE)
{
  args = list(...)
  dfs = lapply(args, function(a) setColnames(as.data.frame(a), c("from", "to")));
  out = removeDuplicatesFromPrettifyList( do.call(rbind, dfs));
  if (order)
  {
    order1 = order(-nchar(out[[1]]));
    out = out[order1, ];
  }
  out;
}
...
#=========================================================================================================
#
# standardized plot of data in groups 
#
#=========================================================================================================
    ...
    ...)
{
  ...
  nSamples = length(data);
  col = .extend(col, nSamples);
  border = .extend(border, nSamples);
  mp = barplot(data[sampleOrder],
          names.arg = rep("", nSamples), #group[sampleOrder], las = 2, 
         col = col[sampleOrder], border = border[sampleOrder],
         ...)
...
)
{
  ...
  {
      out = c(multiGrep(c("^Downregulated", "^down.for"), enrichmentTable$class), 
      ...
      out = c(out, setdiff(1:nrow(enrichmentTable), out));
      out;
  }
  ...
  {
    printFlush("Invalid 'plotComponent'.");
    browser();
  }
  ...
  if (is.null(prettifyList)) prettifyList = analysis$prettifyList.plots else
  {
    ...
    keep = !duplicated(prettifyList[[1]]);
    prettifyList = lapply(prettifyList,`[`, keep);
  } 

  prettifyList = lapply(prettifyList, as.vector)
  if (is.null(enrTab)) browser("enrTab is NULL.")
...
enrichmentBarplot.general = function(
   ...
   featureWord.singular = "gene",
   featureWord.plural = spaste(featureWord.singular, "s"),
   ...
   reformModuleLabels = TRUE,  ## For newer collections this is better set FALSE
...
)
{
  ...
  {
    stop("'thresholdColumn' ", thresholdColumn, " not found among columns in enrichment table. Dropping into browser.");
  }
  ...
  {
    classGenes = classGenes[as.character(tab1[[classNameColumn]])];
  ...
  if (keepClassOnOnePage) 
  {
    ...
    rowsPerPage = allocateJobs(nrow(tab1), nWorkers = ceiling(nrow(tab1)/maxRowsPerPage));
                       ...
                       classBesideBars)
  {
    ...
    if (length(classGenes) > 0)
    {
      ...
      genes = formatLabels(spaste(n1, 
                         ifelse (n1==1, spaste(" ", featureWord.singular, ": "), spaste(" ", featureWord.plural, ": ")), 
  ...
  plotAnnotArgs = list(sepLabels = classes.pretty,
                       ...
                       featureWord.singular = featureWord.singular,
                       featureWord.plural = featureWord.plural,
  ...
  bp = tryCatch(labeledBarplot3.multiPage(lp,
               border = barBorder,
               col = barColor, 
               yLabels = yLabels,
               ...
               ...), 
               error = function(cond) {browser(spaste("labeledBarplot3 returned error: ", cond))},
               finally = NULL);
}
...
rescuePlot.base = function(
   ...
   reduceLargeClasses = FALSE,
   reduce.threshold = 1000,
   reduce.power = 4/5,
   randomSeed = 625,
   ...
)
{
  ...
  if (length(plotIndex)!=n0) stop("Length of 'plotIndex' and 'plotStat' must be the same.");

  if (length(excludeIndex)>0) plotIndex[excludeIndex] = FALSE;

  if (reduceLargeClasses)
  {
    for (cl in classOrder)
    {
      inClass = which(replaceMissing(class==cl, FALSE))
      plotInClass = inClass[replaceMissing(plotIndex[inClass], FALSE)];
      n1 = length(plotInClass);
      if (n1 > reduce.threshold)
      {
        cat(spaste("Reducing size of module ", cl, " from ", n1, " to "));
        drop = sample(plotInClass, n1 - reduce.threshold - round( (n1-reduce.threshold)^reduce.power));
        plotIndex[drop] = FALSE;
        printFlush(spaste(sum(plotIndex[inClass], na.rm = TRUE), " features."));
      }
    }
  }
...
#=========================================================================================================
#
# quantile normalization to a given reference vector
#
#=========================================================================================================

# Here x is assumed to have samples in rows and genes in columns
normalizeQuantilesToReference = function(x, ref)
{
  nSamples = nrow(x);
  nGenes = ncol(x);
  if (length(ref)!=nGenes)
    stop("Length of 'ref' must equal the number of genes (columns) in 'x'.");

  ref.sorted = sort(ref);
  xRank = rowRanks(x, ties.method = "first");
  out = matrix(0, nSamples, nGenes);
  for (s in 1:nSamples)
  {
    out[s, ] = ref.sorted[xRank[s, ]];
  }
  dimnames(out) = dimnames(x);
  out;
}
...
retrieveEnsembleData = function(type = c("Gene", "Transcript", "Protein", "Exon"),
                                organism,
                                version = "current",
                                rootDir = NULL,
...
marginWidth = function(text, cex=1, device = NULL, vfont = NULL, font = NULL, textMargin = 1)
{
  ...
  marginWidth.lines = ceiling(maxWidth.in0/marginLineWidth.in)
...
scpp = function(mainMar = 2, rightMar = 1)
{
  par(mar = c(3.1, 3.1, mainMar, rightMar));
  par(mgp = c(2, 0.7, 0));
}

scpp2 = function(mainMar = 2, rightMar = 1)
{
  par(mar = c(3.4, 3.4, mainMar, rightMar));
  par(mgp = c(2, 0.7, 0));
}
...
changepoints = function(x, first = FALSE, last = FALSE, atEnd = FALSE)
{
  n = length(x);
  out = which(x[-1]!=x[-n]) + 1;
...
traverseConsensusInfo = function(consensusInfo)
{
  ...
  isInfo = !sapply(consensusInfo$consensusTree$inputs, is.character);
  if (any(isInfo))
     out1 = c(out1, unlist(lapply(consensusInfo$consensusTree$inputs[isInfo], traverseConsensusInfo, returnList = TRUE), 
                           recursive = FALSE));  
  ...
  ...)
{
  ...
  keepRows = rowSums(ptab<=includeThreshold) > 0;
  keepCols = colSums(ptab<=includeThreshold) > 0;
  ...
  out = labeledHeatmap(lp,
    ...
    textMatrix = text,
    colors = blueWhiteRed(2*nColorSteps + 1)[(nColorSteps+1):(2*nColorSteps + 1)],
    zlim = c(0, logpLimit),
    setStdMargins = FALSE,
  ...
  ...)
{
  ...
  par(mar = mar);
  out = wgHeatmap(lp,
    colLabels.bottom = colLabels[keepCols],
    colColors.bottom = colColors[keepCols],
    rowLabels.left = rowLabels[keepRows],
    rowColors.left = rowColors[keepRows],
    textMatrix = text,
    colors = blueWhiteRed(100)[50:100],
    zlim = c(0, logpLimit),
    horizontalSeparator.y = rowSep,
...
clipData = function(data, max, min = -max)
{
  data[replaceMissing(data) < min] = min
  data[replaceMissing(data) > max] = max;
  data;
}

clipData.soft = function(data, min, max, d = (max - min)/50)
{
  low = replaceMissing(data < min + d);
  data[low] = min + d - d*tanh((min + d - data[low])/d)
  high = replaceMissing(data > max-d)
  data[high] = max - d + d*tanh((data[high] - max + d)/d);
  data;
}
...
standardizeSampleNames = function(s, extreme = FALSE, makeNames = FALSE) 
{
  if (makeNames) s = make.names(s);
  out = multiGSub(c("^X", "_+", " +", "-+", "\\.+", "\\+"), c("", "_", "_", "_", "_", "_"), s);
  if (extreme) out = tolower(out);
  out;
}
...
#==========================================================================================================
#
# Restrict ordering to a subset
#
#==========================================================================================================

subsetOrdering = function(order, subset)
{
  if (!is.logical(subset))
  {
    x = rep(FALSE, length(order))
    x[subset] = TRUE;
    subset = x;
  }
  shift = c(0, cumsum(!subset));
  order2 = order[ subset[order] ];
  order2 = order2 - shift[order2]
  order2;
}

# Figure out where separators should be so that they fall onto changepoints of labels ordered by the order and restricted
# to a subset
orderedSubsetChangepoints = function(labels, order, subset = NULL)
{
  if (length(labels)==0) return(numeric(0));
  n = length(order);
  i = 1:n;
  if (length(subset)==0) subset = rep(TRUE, n);
  subsetOrder = subsetOrdering(order, subset);
  cp = changepoints(labels[subset][subsetOrder], atEnd = TRUE)
  if (length(cp)==0) return(numeric(0));
  colSep = match( i[subset][subsetOrder] [cp], i[subset]);
  colSep;
}

blueWhiteGreen = function(n, gamma = 1, endSaturation = 1)
{
  if (endSaturation >1  | endSaturation < 0) stop("'endSaturation' must be between 0 and 1.");
  es = 1-endSaturation;
  blueEnd = c(0.05 + es * 0.45 , 0.05 + es * 0.25, 1.00);
  redEnd = c(0.04 + es * 0.6, 0.7, 0.6*es);
  middle = c(1,1,1);
...
escapeSpecialForGrep = function(s, specialChars = c("().|[]+*?"))
{
  nc = nchar(specialChars);
  sc = strsplit(specialChars, split = "")[[1]];
  multiGSub(sc, spaste("\\", sc), s, fixed = TRUE);
}


#==========================================================================================================
#
# match that does not match missing values 
#
#==========================================================================================================

match.na = function(...) match(..., incomparables = NA);


#==========================================================================================================
#
# compress UMAP points to remove empty spaces
#
#==========================================================================================================

removeEmptySpaces = function(x, y, step)
{
  xmin = min(x);
  ymin = min(y);
  xmax = max(x);
  ...
  nx = ceiling((xmax - xmin)/step);
  step1.x = (xmax - xmin)/nx;
  xt = xmin + (c(1:nx)-0.5) * step1.x
  remove.x = rep(TRUE, nx);
  for (i in 1:nx)
    if (any(abs(x-xt[i]) < step1.x)) remove.x[i] = FALSE;

  ny = ceiling((ymax - ymin)/step);
  step1.y = (ymax - ymin)/ny;
  yt = ymin + (c(1:ny)-0.5) * step1.y
  remove.y = rep(TRUE, ny);
  for (i in 1:ny)
    if (any(abs(y-yt[i]) < step1.y)) remove.y[i] = FALSE;
  
  xnew = x;
  ynew = y;
  for (i in which(remove.x))
  {
    change = x > xt[i];
    xnew[change] = xnew[change] - step1.x;
  }
  for (i in which(remove.y))
  {
    change = y > yt[i];
    ynew[change] = ynew[change] - step1.y;
  }

  data.frame(x = xnew, y = ynew);
}

compressUMap = function(umapData, step = 0.2, asp = 0)
{
  cmp = removeEmptySpaces(umapData$x, umapData$y, step = step);
  umapData[c("x", "y")] = cmp;
  if (asp>0)
  {
    xr = range(umapData$x);
    yr = range(umapData$y);
    asp0 = (yr[2] - yr[1])/(xr[2] - xr[1]);
    scale = asp/asp0;
    umapData$y = umapData$y * scale;
  }
  umapData;
};
...
plotUMap = function(
  ...
  closeDevice = TRUE,
  compressed = FALSE,
  width = if (compressed) 10 else 12, height = 7,
  frame = FALSE,
  main = "",
  mar.main = if (main=="") 0.2 else 2, 
  baseCol = "grey60", baseBg = "grey90", 
  mar = c(0.2, 0.2, mar.main, 0.2),
  cex.base = 0.8, pch.base = 21,
  lwd.base = 1,
  umapData = read.csv(gzfile("AuxilliaryData/umap_2020_01_21.csv.gz")),
  ...
  pch.overlay = 21,
  lwd.overlay = 1,
  scramble = TRUE, randomSeed = 123, 
  scrambleBase = FALSE,
  thinnedBase = FALSE,
  thinned.showPoints = NULL,
  xlim = NULL, ylim = NULL, ...)
{
  if (!is.null(file))
  {
    match.fun(device) (file = file, width = width, height = height);
    if (closeDevice) on.exit(dev.off());
    par(mar = mar);
  }
  np = nrow(umapData);
  if (is.character(overlayIndex)) overlayIndex = match(overlayIndex, umapData$GeneName);
  if (any(is.na(overlayIndex))) warning("Not all entries in overlayIndex can be matched (are finite).");
  if (is.logical(overlayIndex)) overlayIndex = which(overlayIndex);
  no = length(overlayIndex);
  overlayCol = .extend(overlayCol, no);
  overlayBg = .extend(overlayBg, no);
  cex.overlay = .extend(cex.overlay, no);
  pch.overlay = .extend(pch.overlay, no);
  lwd.overlay = .extend(lwd.overlay, no);
  keep = is.finite(overlayIndex)
  overlayCol = overlayCol[keep];
  overlayBg = overlayBg[keep];
  cex.overlay = cex.overlay[keep];
  pch.overlay = pch.overlay[keep];
  overlayIndex = overlayIndex[keep];
  baseCol = .extend(baseCol, np);
  baseBg = .extend(baseBg, np);
  lwd.base = .extend(lwd.base, np);
  cex.base = .extend(cex.base, np);
  pch.base = .extend(pch.base, np);
  base = setdiff(1:np, overlayIndex);
  if (is.null(xlim)) xlim = range(umapData$x, na.rm = TRUE);
  if (is.null(ylim)) ylim = range(umapData$y, na.rm = TRUE);
  if (exists(".Random.seed"))
  {
    seed = .Random.seed;
    on.exit( { .Random.seed <<- seed }, add = TRUE);
    set.seed(randomSeed);
  }
  if (scrambleBase) 
  {
    order = sample(1:length(base));
  }  else order = 1:length(base);
  if (thinnedBase) {
     thinnedScatterplot(umapData$x[base][order], umapData$y[base][order],
           xlab = "", ylab = "", xaxt = "n", yaxt = "n",
           cex = cex.base[base][order], pch = pch.base, col = baseCol[base][order], bg = baseBg[base][order], 
           lwd= lwd.base[base][order], showPoints = thinned.showPoints,
           main = main,
           cex.lab = 1, cex.main = 1, cex.axis = 1, frame = frame, xlim = xlim, ylim = ylim, ...);
  } else 
      plot(umapData$x[base][order], umapData$y[base][order],
           xlab = "", ylab = "", xaxt = "n", yaxt = "n",
           cex = cex.base[base][order], pch = pch.base, col = baseCol[base][order], bg = baseBg[base][order], 
           lwd= lwd.base[base][order],
           main = main,
           cex.lab = 1, cex.main = 1, cex.axis = 1, frame = frame, xlim = xlim, ylim = ylim, ...);

  if (length(baseFilter)!=0)
  {
    box = par("usr");
    if (length(baseFilter)==1) col1 = rgb(255, 255, 255, baseFilter, maxColorValue = 255) else
      col1 = rgb(baseFilter, maxColorValue = 255);
    rect(box[1], box[3], box[2], box[4], 
      col = col1, border = NA);
  }

  if (length(overlayIndex)>0)
  {
    if (scramble) {
      order = sample(1:length(overlayIndex));
    } else order = 1:length(overlayIndex);
    points(umapData$x[ overlayIndex[order]], umapData$y[ overlayIndex[order]], 
        pch = pch.overlay[order], cex = cex.overlay[order], col = overlayCol[order], bg = overlayBg[order],
        lwd = lwd.overlay[order]);
  }
}

#===============================================================================================================
#
# Retrieve CoExMap data
#
#===============================================================================================================

retrieveCoExMapData = function(baseDir = "~/Work", 
   CoExMapBase = file.path(baseDir, 
                           "HuntingtonsDisease/IndividualAnalyses/GNVProject/CommonAnalyses/160-WGNCA-perGenotype",
                           "033-NetworkAnalysis-selected-V3"),
   UMapDataFile = file.path(CoExMapBase, "AuxilliaryData/umap_2020_01_21.csv.gz"),
   networkFile = file.path(CoExMapBase, "Results/networkAnalysisResults-WT.main.csv.gz"))
{
  if (!is.null(UMapDataFile)) {
    umapData = read.csv(gzfile(UMapDataFile));
  } else umapData = NULL;

  if (!is.null(networkFile)) {
    net = read.csv(gzfile(networkFile));
  } else net = NULL;

  list(umapData = umapData, net = net);
}

#===============================================================================================================
#
# Color pallette for diurnal sets. 8 colors that are different but represent a sequence
#
#===============================================================================================================

colorSequence.8 = function()
{
  colors0 = rainbow(9)[-4];
  colors0[c(1, 6)] = c("#FF7777FF", "#7777FFFF");
  colors1 = colors0[ c(7, 8, 1:6)];
  colors1;
}

#===============================================================================================================
#
# synthetize module representative from data and kME values
#
#===============================================================================================================

# note: we have a function called moduleRepresentatives but it does soemthing different.

weightedModuleRepresentatives = function(data, labels, kME, power, excludeGrey, grey = if (is.numeric(labels)) 0 else "grey",
                                           scale = TRUE, colnamePrefix = "ME")
{
  modLevels = sort(unique(labels));
  if (excludeGrey) modLevels = setdiff(modLevels, grey);

  if (scale) data = scale(data);
  weights = abs(kME) ^ power;

  nModules = length(modLevels);
  MEs = matrix(0, nrow(data), nModules)
  for (m in 1:nModules)
  {
    keep = labels==modLevels[m];
    repr = rowWeightedMeans( data[, keep, drop = FALSE], weights[keep], na.rm = TRUE)
    MEs[, m] = repr;
  }
  colnames(MEs) = spaste(colnamePrefix, modLevels);
...
darkenLightColors = function(col, threshold = 190, minDif = 100, darkenGreen = FALSE)
{
  rgb = col2rgb(col);
  mean = colMeans(rgb);
  r = mean/threshold;
  r[ r < 1] = 1;
  rgb2 = rgb / matrix(r, nrow(rgb), ncol(rgb), byrow = TRUE)/255
  # Further darken pure green
  if (darkenGreen) rgb2[2, rgb2[2, ] > threshold/255] = threshold/255;
  rgb(rgb2[1, ], rgb2[2, ], rgb2[3, ], maxColorValue = 1);
}

saturateColors = function(col, multiplier = 1.2)
{
  rgb = col2rgb(col);
  hsv1 = rgb2hsv(rgb);
  s = multiplier * hsv1[2, ];
  s[s>1] = 1;
  hsv1[2, ] = s;
  hsv(h = hsv1[1, ], s = hsv1[2, ], v = hsv1[3, ]);
}
...
if (FALSE)
{
   ...
   par(mar = c(0,0,0,0));
   n = 30;
   barplot(rep(1, n), col = standardColors(n));
   barplot(rep(1, n), col = saturateColors(standardColors(n), multiplier = 2));
}

#===============================================================================================================
#
# Circular circadian plot
#
#===============================================================================================================

circularCircadianPlot = function(time, data,
  type = c("polygon", "lines", "points", "both", "bars"),
  col, border, pch = 1, ptCex = 1,
  lnLwd = 1, ptLwd = 1,
  pgCol = col,
  pgBorder = border,
  ptCol = border,
  ...
  radGrid = TRUE,
  cex.axis = 1,
  tickLen = 0.5*strwidth("M"),
  axisCol = "grey30",
  shiftAxisLabels = 1,   ## This is in units of strwidth("M")
  axisLabelDir = 1,  ## 1 means to the right in the direction of the axis, -1 to the left
  ...)
{
  type = match.arg(type);
  pin = par("pin");
  ...
  if (is.null(timeLabelPos))
  {
    if (length(time) <= 8) timeLabelPos = time;
  }
  if (is.logical(timeLabels) && !timeLabels)
    timeLabelPos = NULL;
  if ( (is.logical(timeLabels) && timeLabels) || is.null(timeLabels)) timeLabels = spaste(timeLabelPos, " h");

  if (startNewPlot)
  {
    plot(c(-1, 1), c(-asp, asp), type = "n", xlab = "", ylab = "", axes = FALSE, ...);
    radius1 = radius * min(1, asp);
  } else
    radius1 = radius;
  ...
  box = par("usr");
  asp1 = asp * (box[2] - box[1])/(box[4] - box[3]);

  if (!is.null(timeLabelPos))
  {
    timeLabW = max(strwidth(timeLabels)) + 0.2*strwidth("M");
    timeLabH = max(strheight(timeLabels)) + 0.2*strheight("M")
    radius1 = radius * min(1 - timeLabW/(box[2] - box[1]), 
                 pin[2]  * (1-timeLabH/(box[4] - box[3])) / pin[1]); ##(pin[1] * (1-timeLabW/(box[2]-box[1]))));
  } 

  radius.x = radius1;
  radius.y = radius.x * asp1;

  dayAngles = seq(from = angle.0h, to = angle.0h - 180, by = -1) * pi/180;
  nightAngles = seq(from = angle.0h-180, to = angle.0h - 360, by = -1) * pi/180;
  if (startNewPlot)
  {
  polygon(x = center.x1 + radius.x * cos(dayAngles), y = center.y1 + radius.y * sin(dayAngles),
               col = dayBg, border = dayBg);
  polygon(x = center.x1 + radius.x * cos(nightAngles), y = center.y1 + radius.y * sin(nightAngles),
               col = nightBg, border = nightBg);
  }

  if (is.null(lim)) lim = range(data, na.rm = TRUE)
  if (limStretch!=0) lim = stretchLim(lim, limStretch);

  min = lim[1];
  max = lim[2];
  scaledData = (data - min)/(max - min) * radius1;
  dataAngles = angle.0h * pi/180 - (time %% 24) * pi/12;
  dataOrder = order(time %% 24);
  data.o = scaledData[dataOrder];
  angles.o = dataAngles[dataOrder];
  x = center.x1 + data.o * cos(angles.o);
  y = center.y1 + asp1*data.o * sin(angles.o);
  if (type=="polygon")
    polygon( x = x, y = y, col = col, border = border);

  if (type %in% c("lines", "both"))
    lines( x = c(x, x[1]), y = c(y, y[1]), col = lnCol, lwd = lnLwd);
           
  if (type %in% c("points", "both"))
    points( x = x, y = y, pch = pch, cex = ptCex, col = ptCol, bg = ptBg, lwd = ptLwd);

  if (type == "bars")
  {
    time.24 = time[dataOrder] %% 24;
    time.sep = (time.24 + c(time.24[length(time.24)]-24, time.24[-length(time.24)]))/2
    time.sep = c(time.sep, time.sep[1]);
    time.sep.lst = lapply(1:length(time), function(i) time.sep[c(i, i+1)]);
    angles.sep.lst = lapply(time.sep.lst, function(t) angle.0h * pi/180 - (t%%24) * pi/12);
    mymapply(function(ang, x) 
    {
      if (ang[1] < ang[2]) ang[1] = ang[1] + 2*pi;
      angles = seq(from=ang[1], to = ang[2], length.out = 360/length(time));
      polygon( center.x1 + c(0, x * cos(angles)), 
               center.y1 + c(0, x*sin(angles)),
               col = col, border = border)
    }, angles.sep.lst, data.o);
  }

  
  tickPos = pretty(lim, n = nTicks)
  tickPos = tickPos[ tickPos > lim[1]  & tickPos <= lim[2]];
  tickPos.scaled = (tickPos- min)/(max - min) * radius1

  if (tanGrid)
  {
    angles1 = seq(from = 0, to = 360, by = 1)/360 * 2*pi;
    keepTicks = tickPos > lim[1] & tickPos < lim[2];
    lapply(tickPos.scaled[keepTicks], function(r) 
      lines(center.x1 + r * cos(angles1), center.y1 + asp1 * r *sin(angles1), col = tanLineCol));
  }
  ...
  if (radGrid)
  {
    if (type == "bars")
    {
      lapply(angles.sep.lst, function(an) 
         lines(center.x1 + c(0, 1) * radius1 * cos(an[1]), center.y1 + c(0,1) * asp1 * radius1 * sin(an[1]),
            col = radLineCol));
    } else 
      lapply(dataAngles, function(a)
        lines(center.x1 + c(0, 1) * radius1 * cos(a), center.y1 + c(0,1) * asp1 * radius1 * sin(a),
              col = radLineCol));
  }

  if (axis)
  {
    textPos = lapply(tickPos.scaled, function(pos) {
      x0 = center.x1 + pos * cos(axisAngle * pi/180);
      y0 = center.y1 + pos * asp1 * sin(axisAngle * pi/180);
      angle = axisAngle-90;
      sx = tickLen * cos(angle * pi/180);
      sy = tickLen * asp1 * sin(angle* pi/180);
      lines(c(x0-sx, x0 + sx), c(y0-sy, y0+sy), col = axisCol);
      sx2 = axisLabelDir * (tickLen + 0.2 * strwidth("M")) * cos(angle * pi/180) ;
      sy2 = axisLabelDir * (tickLen + 0.2 * strwidth("M")) * asp1 * sin(angle * pi/180);
      if (shiftAxisLabels !=0) 
      {
        sx2 = sx2 + shiftAxisLabels * strheight("M") * cos(axisAngle * pi/180)
        sy2 = sy2 + shiftAxisLabels * strheight("M") * sin(axisAngle * pi/180)
      }
      c(x0 + sx2, y0 + sy2);
    });
    axisAngle = axisAngle %% 360;
    tickDir = if (axisAngle > -10 && axisAngle<10) 0 else if (axisAngle > 170 && axisAngle<190) 1 else 0.5;
    mymapply(function(pos, txt) text(pos[1], pos[2], txt,
        adj = c( if (tickDir==0.5) (1-axisLabelDir)/2 else 0.5, tickDir)),
    textPos, tickPos);
  }

  if (length(timeLabelPos) > 0)
  {
    if (length(timeLabels)!=length(timeLabelPos))
      stop("Length of 'timeLabels' and 'timeLabelPos' must be the same.");
    w = strwidth(timeLabels)/2
    h = strheight(timeLabels)/2/asp1
    labAngles = -(timeLabelPos %%24) * pi/12 + angle.0h * pi/180;
    tg = tan(labAngles)
    tg[tg > 1e10] = 1e10;
    hw = h/w;
    labSize = abs(ifelse(abs(tg) > hw, h/sin(labAngles), w/cos(labAngles))) + 0.2 * strwidth("M")
    r = radius1;
    labx = center.x + r*cos(labAngles) + labSize * cos(labAngles);
    laby = center.y + asp1 * r*sin(labAngles) + asp1 * labSize * sin(labAngles);
    text(labx, laby, timeLabels, col = axisCol, xpd = TRUE, adj = c(0.5, 0.5));
  }
    
  invisible(list(center.x = center.x1, center.y = center.y1, 
                 x = x, y = y,
                 radius = radius1));
}
...
#===============================================================================================================
#
# select top genes from a set of DE genes
#
#===============================================================================================================

selectTopGenes = function(selectionStat, threshold, 
          addlStat1 = NULL, addlThreshold1 = NULL, 
          addlStat2 = NULL, addlThreshold2 = NULL,
          maxN, minN)
...
#===============================================================================================================
#
# calculate optimal ellipse to enclose a set of points
# This is not finished and not used right now. There are existing functions in standard libraries that can be used to
# compute the spanning ellipse.
#
#===============================================================================================================

# For a given center and angle of the main axis, calculate the radii of ellipse to enclose the two points
ellipseFor2points = function(
  # Center of ellipse 
  cx, cy, 
  # Angle of one of the axes in radians
  angle, 
  # 2 points given as 2-component vectors
  x, y,
  # Constant A in the ellipse equaltion x^2/a^2 + y^2/b^2 = A
  A = 1.1)
{
  xs = x^2;
  ys = y^2;
  ...
  ys1 = ys[1];
  ys2 = ys[2];
  if ((xs1==xs2) || (ys1==ys2) || (ys1 - ys2)/(xs1 - xs2) < 0)
  {
    as = bs = max((xs1 + ys1), (xs2 + ys2))/A;
  } else {
    D = xs2 * ys2 - xs1 * ys2;
    as = A * (xs2 - xs1)/D;
    bs = A * (ys1 - ys2)/D;
...
#===============================================================================================================
#
# star barplot/ star plot
#
#===============================================================================================================

starBarplot = function(height, Rin, Rout, width, color, border, lim = NULL,
   labels.in = NULL, labels.out = NULL, startAngle, endAngle = startAngle + 360, angleDir = 1,
   add = FALSE, center.x = 0, center.y = 0, ...)
{
  n = length(height);
  if (is.null(lim)) lim = c(0, max(height, na.rm = TRUE));
  Rdif = Rout - Rin;
  height2 = Rin + Rdif * ( (height - lim[1])/(lim[2] - lim[1]))
  angleStep = (endAngle-startAngle)/n;
  angles = 2*pi * angleDir * (( 0: (n-1)) * angleStep + startAngle)/360;
  if (!add) plot(c(-1, 1), c(-1, 1), xlab = "", axes = FALSE, type = "n", ...);
  pin = par("pin");
  box = par("usr");
  rx = box[2]- box[1];
  ry = box[4] - box[3];
  asp = pin[1]/pin[2] * ry/rx;
  scale = 1;
  if (asp > 1) {scale = 1/asp; asp = 1}
  color = .extend(color, n);
  border = .extend(border, n);
  
  for (i in 1:n) 
  {
    if (!is.na(height[i]))
    {
      coords = trafo( x = c(Rin, Rin, height2[i], height2[i]), y = c(-width/2, width/2, width/2, -width/2),
                 center.x = center.x, center.y = center.y, angle = angles[i], scale = scale, asp = asp);
      polygon(coords$x, coords$y, border = border[i], col = color[i]);
    }
    if (!is.null(labels.out))
    {
      tc = trafo( x = height2[i] + strwidth("M")/2, y = 0,
                 center.x = center.x, center.y = center.y, angle = angles[i], scale = scale, asp = asp);
      text(tc$x, tc$y, labels.out[i], xpd = TRUE, 
           adj = c( if (cos(angles[i]) > 0.05) 0 else if (abs(cos(angles[i]))<0.05) 0.5 else 1,
                    if (sin(angles[i]) > 0.05) 0 else if (abs(sin(angles[i]))<0.05) 0.5 else 1));
    }
    if (!is.null(labels.in))
    {
      tc = trafo( x = Rin - strwidth("M")/2, y = 0,
                 center.x = center.x, center.y = center.y, angle = angles[i], scale = scale, asp = asp);
      text(tc$x, tc$y, labels.in[i], xpd = TRUE,
           adj = c( if (cos(angles[i]) > 0.05) 1 else if (abs(cos(angles[i]))<0.05) 0.5 else 0,
                    if (sin(angles[i]) > 0.05) 1 else if (abs(sin(angles[i]))<0.05) 0.5 else 0));
    }
  }
}
...
#
#===============================================================================================================
# Approximate test for difference in enrichment

sampleWithEnrichmentRatio = function(objects, size, er)
{
  p1 = 1/(er + 1)
  p2 = er/(er + 1);
  prob = c(p1, p2)[objects];
  sample(objects, size = size, prob = prob);
}

differentialEnrichmentAnalysis = function(
  # a (number of sets) x 2 matrix
  overlapSizes,
  # numbers of genes in the two classes (scalars)
  n1, n2,
  # Vector of number of genes in reference sets
  setSizes,
  # Scalar or vector of background sizes
  backgroundSize,
  checkInput = TRUE)
{
  ...
  if (checkInput)
  {
    if (is.null(dim(overlapSizes))) overlapSizes = t(as.matrix(overlapSizes));
    if (!isTRUE(all.equal(dim(overlapSizes), c(nSets, 2)))) 
      stop("'overlapSizes' must be a (number of sets) times 2 matrix.");
    if (any(n1 < overlapSizes[, 1] | overlapSizes[, 1] > setSizes))
      stop("Number of overlapping genes must be less than corresponding class and set sizes (component 1).");
    if (any(n2 < overlapSizes[, 2] | overlapSizes[, 2] > setSizes))
      stop("Number of overlapping genes must be less than corresponding class and set sizes (component 2).");
  }
  #setComplementSizes = backgroundSize - setSizes;
  #r1 = overlapSizes[, 1] * setComplementSizes / (setSizes * (n1-overlapSizes[, 1]));
  #r2 = overlapSizes[, 2] * setComplementSizes / (setSizes * (n2-overlapSizes[, 2]));
  #var1 = overlapSizes[, 1] * ( setComplementSizes / (setSizes * (n1-overlapSizes[, 1])))^2
  #var2 = overlapSizes[, 2] * ( setComplementSizes / (setSizes * (n2-overlapSizes[, 2])))^2;

  r1 = overlapSizes[, 1] * backgroundSize / (setSizes * n1)
  r2 = overlapSizes[, 2] * backgroundSize / (setSizes * n2)
  p1 = overlapSizes[, 1]/n1;
  p2 = overlapSizes[, 2]/n2;
  var1 = overlapSizes[, 1] * (1-p1) * ( backgroundSize / (setSizes * n1))^2
  var2 = overlapSizes[, 2] * (1-p2) * ( backgroundSize / (setSizes * n2))^2
  w1 = 1/var1;
  w2 = 1/var2;
  means = (w1 * r1 + w2 * r2)/(w1 + w2);
  Z = abs(r1-means)/var1 * sqrt(var1 + var2);
  2*pnorm(Z, lower.tail = FALSE);
} 

if (FALSE)
{
  # Test the significance of differences in enrichment
  nRuns = 10000;
  simulatedER = 2;
  pvalues = numeric(nRuns);
  pind = initProgInd();
  B1 = 200; B2 = 600; A = 10000;
  overlapSizes = matrix(0, nRuns, 2);
  n1 = n2 = setSizes = numeric(nRuns);
  p1 = 0.3
  for (r in 1:nRuns)
  {
    #if (r%%100==1)
    ...
    {
      objects = rep(1, A);
      objects[sample(A, size = p1*A)] = 2
    }
    class1 = sampleWithEnrichmentRatio(objects, B1, simulatedER);
    class2 = sampleWithEnrichmentRatio(objects, B2, simulatedER);
    classes = list(class1, class2);
    overlapSizes[r, ] = sapply(classes, function(x) sum(x == 2))
    n1[r] = length(class1);
    n2[r] = length(class2);
    setSizes[r] = sum(objects==2);
    pind = updateProgInd(r/nRuns, pind);
  }
  pvalues = differentialEnrichmentAnalysis(overlapSizes = overlapSizes, n1 = n1, n2 = n2, setSizes = setSizes, backgroundSize = A);
...
#===============================================================================================================
#
# flatten a 2-dimensional table to a linear vector
#
#===============================================================================================================

linearizeMatrix = function(mat, rowName = "row", colName = "column", dataName = "data")
{
  cols = rep(if (is.null(colnames(mat))) 1:ncol(mat) else colnames(mat), each = nrow(mat));
  rows = rep(if (is.null(rownames(mat))) 1:nrow(mat) else rownames(mat), ncol(mat));
  setNames(data.frame(row = rows, column = cols, data = as.vector(mat)), c(rowName, colName, dataName));
}
...
#===============================================================================================================
#
# modified boxplot from graphics:::boxplot.default
#
#===============================================================================================================

PLBoxplot = function (x, ..., range = 1.5, width = NULL, varwidth = FALSE, 
    ...
    at = NULL, plotOutliers = TRUE) 
{
    ...
    if (plot) {
        if (is.null(pars$boxfill) && is.null(args$boxfill)) 
            pars$boxfill <- col
        do.call("PLBxp", c(list(z, notch = notch, width = width, 
            varwidth = varwidth, log = log, border = border, 
            pars = pars, outline = outline, horizontal = horizontal, 
            add = add, ann = ann, at = at, plotOutliers = plotOutliers), args[namedargs]))
        invisible(z)
    }
    else z
}
...
PLBxp = function (z, notch = FALSE, width = NULL, varwidth = FALSE, outline = TRUE, 
    notch.frac = 0.5, log = "", border = par("fg"), pars = NULL, 
    frame.plot = axes, horizontal = FALSE, ann = TRUE, add = FALSE, 
    at = NULL, show.names = NULL, plotOutliers = TRUE, ...) 
{
    pars <- as.list(pars)
    if (...length()) {
    ...
    bplt <- function(x, wid, stats, out, conf, notch, xlog, i) {
        ...
        if (!anyNA(stats)) {
            ...
            if ((nout <- length(out))) {
                ...
                if (plotOutliers) xypoints(rep.int(x, nout), out, pch = outpch[i], 
...
#=================================================================================================================
#
# DESeq2-like normalization
#
#=================================================================================================================

sizeFactors.general = function(expr, exprIsLogTransformed = FALSE, logShift = 0)
{
  if (exprIsLogTransformed) lexpr = expr else lexpr = log(expr + logShift);
  means = colMeans(lexpr, na.rm = TRUE);
  keepGenes = means > 0; 
  if (sum(keepGenes)==0) 
    stop("All genes have at least one zero. Use a non-zero logShift (or check exprIsLogTransformed).");
  x = lexpr[, keepGenes] - matrix(means, nrow(lexpr), sum(keepGenes), byrow = TRUE);
  out = rowMedians(x);
  if (!exprIsLogTransformed) out = exp(out);
  out;
}

medianOfLogFoldChangeNorm = function(expr, exprIsLogTransformed = FALSE, logShift = 1,
   sizeFactors = sizeFactors.general(expr, exprIsLogTransformed, logShift))
{
   if (exprIsLogTransformed) expr-sizeFactors else expr/sizeFactors
}


#=================================================================================================================
#
# Map (genomic) features to genes
#
#=================================================================================================================

mapFeaturesToGenes = function(
   featureAnnotation,
   fChrColumn = "Chromosome",
   fStartColumn = "Start",
   fEndColumn = "End",
   fIDColumn = "ID",

   geneAnnotation,
   gChrColumn = "Chromosome",
   gStartColumn = "Start",
   gEndColumn = "End",
   gStrandColumn = NULL,
   gIDColumn = "GeneID",
   
   method = c("basal + extension", "two nearest genes", "single nearest gene"),
   proximalUpstreamLimit = 5000,
   proximalDownstreamLimit = 1000,
   distalLimit = 1e6,

   includeGColumns = c(gIDColumn, gChrColumn, gStartColumn),
   includeFColumns = c(fIDColumn, fChrColumn, fStartColumn),
   maxNearGenes = 1000,
   verbose = 1)
{
  method = match.arg(method);
  method.i = match(method, c("basal + extension", "two nearest genes", "single nearest gene"));
  chrLevels = unique(geneAnnotation[[gChrColumn]]);
  if (length(chrLevels)==0) stop("Chromosome column ", chrColumn, " within 'geneAnnotation' is empty.");
  out = NULL;
  annotByChr = lapply(chrLevels, function(chr)
  {
    if (verbose > 0) printFlush(spaste("Working on chromosome ", chr));
    gIndex = which(geneAnnotation[[gChrColumn]]==chr);
    geneAnnot1 = geneAnnotation[ gIndex, ];
    fIndex = which(featureAnnotation[[fChrColumn]] == chr);
    featureAnnot1 = featureAnnotation[ fIndex, ];
    gorder = order(abs(geneAnnot1[[gStartColumn]]))
    forder = order(abs(featureAnnot1[[fStartColumn]]));

    gi = 1;
    gstd = geneAnnot1[[gStartColumn]] [gorder];
    gst = abs(gstd[1]);

    nGenes1 = length(gorder);
    nFeatures1 = length(forder);

    nearGeneOrdIndex = nearGeneDst = nearGeneDir = numeric(maxNearGenes);  
            ### hopefully no more than 1000 genes within the max distance
    selectGenes = rep(FALSE, maxNearGenes);
    selectedGenes.lst = vector(mode = "list", length = nFeatures1);
    for (fi in forder)
    {
      st = featureAnnot1[[fStartColumn]][fi];
      while (gst < st && gi <= nGenes1) { gi = gi + 1; if (gi <= nGenes1) gst = abs(geneAnnot1[[gStartColumn]] [gorder[gi]]); }

      # Get the nearest genes up to distal limit, then interpret them.
      dstLim = distalLimit + max(proximalUpstreamLimit, proximalDownstreamLimit);
      nNearGenes = 0; nearestUpstreamGene = 0;
      # look down
      if (gi > 1)
      {
        gi1 = gi - 1;
        gst1 = abs(geneAnnot1[[gStartColumn]] [gorder[gi1]]);
        dst = abs(st - gst1);
        while(dst < dstLim && gi1 > 0)
        {
           nNearGenes = nNearGenes + 1;
           nearGeneOrdIndex[nNearGenes] = gi1;
           nearGeneDst[nNearGenes] = dst;
           nearGeneDir[nNearGenes] = -1;
           gi1 = gi1 -1;
           if (gi1 > 0)
           {
             gst1 = abs(gstd[gi1]);
      ...
      if (gi <= nGenes1)
      {
        ...
        gst1 = abs(geneAnnot1[[gStartColumn]] [gorder[gi1]]);
        dst = abs(st - gst1);
        while(dst < dstLim && gi1 <= nGenes1)
        {
           nNearGenes = nNearGenes + 1;
           nearGeneOrdIndex[nNearGenes] = gi1;
           nearGeneDst[nNearGenes] = dst;
           nearGeneDir[nNearGenes] = 1;
           if (nearestUpstreamGene==0) nearestUpstreamGene = nNearGenes;
           gi1 = gi1 +1;
           if (gi1 <= nGenes1)
           {
             gst1 = abs(gstd[gi1]);
      ...
      if (nNearGenes==0) next;
      selectGenes = rep(FALSE, nNearGenes);
      nearGeneDst1 = nearGeneDst[1:nNearGenes];
      nearGeneOrdIndex1 = nearGeneOrdIndex[1:nNearGenes];
      nearGeneDir1 = nearGeneDir[1:nNearGenes];
      nearGeneSt = gstd[nearGeneOrdIndex1];
      if (method=="basal + extension")
      {
        basal.upstr = (nearGeneDst1 <= proximalUpstreamLimit) & 
                     ( ( nearGeneDir1 ==-1 & nearGeneSt < 0 ) | (nearGeneDir1 ==1 & nearGeneSt > 0 ) )
        basal.downstr = (nearGeneDst1 <= proximalDownstreamLimit) & 
                     ( ( nearGeneDir1 ==-1 & nearGeneSt > 0 ) | (nearGeneDir1==1 & nearGeneSt < 0 ))
        basal = basal.upstr | basal.downstr;
        # If the feature is in any of the basal regions, it cannot be in an extension region which is defined to exclude
        # all basal regions
        if (any(basal)) {
          selectGenes = basal;
        } else {
          if (nearGeneDir1[1]==-1 && nearGeneDst1[1] <= distalLimit) selectGenes[1] = TRUE;
          if (nearestUpstreamGene > 0 && nearGeneDst1[nearestUpstreamGene] <=distalLimit) 
            selectGenes[nearestUpstreamGene] = TRUE
        }
      }
      if (method=="two nearest genes")
      {
        if (nearGeneDir[1]==-1 && nearGeneDst1[1] <= distalLimit) selectGenes[1] = TRUE;
        if (nearestUpstreamGene > 0 && nearGeneDst1[nearestUpstreamGene] <=distalLimit)
          selectGenes[nearestUpstreamGene] = TRUE
      }
      if (method=="single nearest gene")
      {
        selectGenes[which.min(nearGeneDst1)] = TRUE;
      }
      selectedGenes.lst[[fi]] = cbind(rep(fIndex[fi], sum(selectGenes)), gIndex[ gorder[ nearGeneOrdIndex1 [selectGenes] ] ] );
      #if (sum(selectGenes) > 2) 
      #{
      #  browser()
      #  sum(selectGenes)
      #  featureAnnot1[fi, ]
      #  geneAnnot1[gorder[ nearGeneOrdIndex1 [selectGenes] ], ]
      #  match(fi, forder)
      #}
    }
    out = selectedGenes.lst[ sapply(selectedGenes.lst, length) > 0];
    if (length(out) > 0) {
      setColnames(do.call(rbind, out), c("featureIndex", "geneIndex"));
    } else 
      NULL
  })
  annot1 = annotByChr[sapply(annotByChr, length) > 0];
  if (length(annot1) > 0)
  {
    index = do.call(rbind, annotByChr[sapply(annotByChr, length) > 0]);
    setNames(data.frame(featureAnnotation[index[, 1], includeFColumns, drop = FALSE],
                        geneAnnotation[index[, 2], includeGColumns, drop = FALSE]), 
               make.unique(c(includeFColumns, includeGColumns)));
  } else NULL;
}        

# Summarize the result of mapFeaturesToGenes such that each row corresponds to a single feature

compressMapToFeatures = function(map, featureIDs = map[[1]],
   featureIDColumn = 1,
   keepFeatureColumns = c("ID", "Chromosome", "Start", "End"),
   keepGeneColumns = c("GeneID", "Entrez", "Symbol"),
   sep = "; ")
{
  uniqueIDs = unique(featureIDs);

  f2map = match(uniqueIDs, map[[featureIDColumn]]);
  featureData = map[f2map, c(keepFeatureColumns, keepGeneColumns)];
  if (any(is.na(f2map)))
    featureData = data.frame.ncn(GivenFeatureID = uniqueIDs, featureData);
  featureData$nMappedGenes = 1;
  map2 = map;
  step = 1;
  while (any(!is.na(f2map)))
  {
    map2 = map2[-f2map[which(!is.na(f2map))], ];
    f2map = match(uniqueIDs, map2[[featureIDColumn]]);
    mapped = !is.na(f2map)
    if (sum(mapped) > 0)
    {
      for (col in keepGeneColumns)
        featureData[[col]] [mapped] = spaste(featureData[[col]] [mapped], sep, map2[[col]] [f2map[mapped]]);
      featureData$nMappedGenes[mapped] = featureData$nMappedGenes[mapped] + 1;
    }
    step = step + 1;
    printFlush("Step", step, ":", nrow(map2), "rows remaining");
  }
...
#=================================================================================================================
#
# Map (genomic) features to another set
#
#=================================================================================================================

mapFeaturesToFeatures = function(
   refAnnotation,
   refChrColumn = "Chromosome",
   refStartColumn = "Start",
   refEndColumn = "End",
   refIDColumn = "ID",

   targetAnnotation,
   tarChrColumn = refChrColumn,
   tarStartColumn = refStartColumn,
   tarEndColumn = refEndColumn,
   tarIDColumn = refIDColumn,

   maxDist = 0,

   includeColumns.ref = colnames(refAnnotation),
   includeColumns.target = colnames(targetAnnotation),
   targetSuffix = ".target",
   addSuffixToAllColumns = FALSE,
   verbose = 1)
{
  chrLevels = unique(refAnnotation[[refChrColumn]]);
  if (length(chrLevels)==0) stop("Chromosome column ", refChrColumn, " within 'refAnnotation' is empty.");
  out = NULL;
  mapByChr = lapply(chrLevels, function(chr)
  {
    if (verbose > 0) printFlush(spaste("Working on chromosome ", chr));
    tarIndex = which(targetAnnotation[[tarChrColumn]]==chr);
    tarAnnot1 = targetAnnotation[ tarIndex, ];
    refIndex = which(refAnnotation[[refChrColumn]] == chr);
    refAnnot1 = refAnnotation[ refIndex, ];
    torder = order(abs(tarAnnot1[[tarStartColumn]]))
    rorder = order(abs(refAnnot1[[refStartColumn]]));

    ti = 1;
    tstd = tarAnnot1[[tarStartColumn]] [torder];
    tend = tarAnnot1[[tarEndColumn]] [torder];
    tst = abs(tstd[1]);
    ten = abs(tend[1]);

    nTargets1 = length(torder);
    nRefs1 = length(rorder);
    map = rep(NA, nRefs1);
    dist = rep(NA, nRefs1);
    for (fi in rorder)
    {
      st = refAnnot1[[refStartColumn]][fi];
      en = refAnnot1[[refEndColumn]][fi]
      ten = abs(tend[ti]);
      tst = abs(tstd[ti]);
      while (ten < st-maxDist && ti <= nTargets1) { ti = ti + 1; if (ti <= nTargets1) ten = abs(tend[ti]); }
      if (ti<= nTargets1)
      {
        ti2 = ti;
        dist1 = maxDist + 1;
        tst = abs(tstd[ti]);
        while (tst <= en + maxDist && ti2 <= nTargets1) { 
          if (ti2 <= nTargets1) {
            overlapEnd = if ( ten >=st && ten <=en ) ten else if (ten > en) en else NA;
            overlapStart = if (tst >=st && tst <= en) tst else if (tst < st) st else NA;
            overlapLength = replaceMissing(overlapEnd - overlapStart + 1, 0);
            overlapDist = if (overlapLength >0) -overlapLength else min(abs(c(ten - st, ten - en, tst - st, tst -en))-1);
            if (overlapDist < dist1) {
              map[fi] = tarIndex[torder[ti2]];
              dist[fi] = overlapDist;
              dist1 = overlapDist;
            }
          } 
          ti2 = ti2 + 1; if (ti2 <= nTargets1) { tst = abs(tstd[ti2]); ten = abs(tend[ti2]) }
        }
      }
    }
    data.frame(ref = refIndex, map = map, dist = dist);
  });

  tarAnnotation2 = targetAnnotation[, includeColumns.target];
  commonNames = if (addSuffixToAllColumns) colnames(targetAnnotation) else 
           intersect(includeColumns.ref, colnames(tarAnnotation2));
  colnames(tarAnnotation2)[ match(commonNames, colnames(targetAnnotation))] = spaste(commonNames, targetSuffix);

  combinedIDs = unlist(lapply(mapByChr, function(map1) refAnnotation[map1$ref, refIDColumn]));

  combinedAnnot = do.call(rbind, lapply(mapByChr, function(map1)
  {
    tryCatch(data.frame.ncn(refAnnotation[map1$ref, includeColumns.ref], target.reference.distance = map1$dist, 
                   tarAnnotation2[map1$map, ]),
        error = function(...) browser(...));
  }));

  combinedAnnot[ match(refAnnotation[[refIDColumn]], combinedIDs), ];
}        
...
{
  n1 = 10
  annot1 = data.frame(ID = 1:n1, Chromosome = 1, Start = (1:n1)*1000, End = (1:n1)*1000 + 50);
  annot2 = data.frame(ID = 1:(3*n1), Chromosome = 1, 
      Start = rep((1:n1)*1000, each = 3) - rep(10*(1:n1), each = 3) + rep(c(0, 5, 10), n1), 
      End = rep((1:n1)*1000, each = 3) - rep(10*(1:n1), each = 3) + rep(c(0, 5, 10), n1) + 50);

  mapFeaturesToFeatures(refAnnotation = annot1, targetAnnotation = annot2, maxDist = 20)
}

#=================================================================================================================
#
# Fix mangled gene symbols
#
#=================================================================================================================

fixGeneSymbols = function(s, organism = "mouse")
{
  fix = multiGrep(spaste("^[0-9]+-", c("Mar", "Sep")), s, ignore.case = TRUE);
  split = strsplit(s[fix], split = "-");
  split = lapply(split, multiSub, pattern = c("^Mar$", "^Sep$"), replacement = c("March", "Sept"), ignore.case = TRUE)
  split = lapply(split, function(s) { s[1] =  as.character(as.numeric(s[1])); s })
  s2 = sapply(split, function(sp) paste(rev(sp), collapse = ""));
  if (organism=="human") s2 = toupper(s2);
  s[fix]= s2;

  if (organism == "mouse")
  {
    fix2 = multiGrep(c("^MARCH", "^SEPT"), s)
    s[fix2] = multiSub(c("MARCH", "SEPT"), c("March", "Sept"), s[fix2]);
  }

  fix3 = multiGrep(c("^Mar[0-9]", "^Sep[0-9]"), s)
  s[fix3] = multiSub(c("Mar", "Sep"), c("March", "Sept"), s[fix3]);

  s;
}

fixGeneColnames = function(x, organism = "mouse")
{
  setColnames(x, fixGeneSymbols(colnames(x), organism = organism))
}


#=================================================================================================================
#
# Identify genes close to a reference
#
#=================================================================================================================

featuresCloseToReference = function(ref, annotation,
   IDColumn = "Symbol", 
   ...
   stopOnNoMatch = TRUE)
{
  if (is.character(IDColumn) && !(IDColumn %in% names(annotation)))
    stop("Cannot find ", IDColumn, " among names of 'annotation'.")
  if (is.character(chrColumn) && !(chrColumn %in% names(annotation)))
    stop("Cannot find ", chrColumn, " among names of 'annotation'.")
  if (is.character(bpColumn) && !(bpColumn %in% names(annotation)))
    stop("Cannot find ", bpColumn, " among names of 'annotation'.")
  refi = match(ref, annotation[[IDColumn]]);
  if (is.na(refi)) 
  {
     if (stopOnNoMatch) stop("Could not find a match for ", ref, " in 'annotation' column ", IDColumn);
     return(numeric(0))
  }
  refChr = annotation[[chrColumn]] [refi];
  refBp = annotation[[bpColumn]] [refi];
  ind = which(annotation[[chrColumn]]==refChr & abs(annotation[[bpColumn]]-refBp) <=maxDist);
  if (returnIndex) ind else annotation[[IDColumn]] [ind];
}

#=================================================================================================================
#
# trait level contrasts
#
#=================================================================================================================
# Given a vector of levels, create a data frame of contrast numerators and denominators

traitLevelContrasts = function(x)
{
  mat = matrix(x, length(x), length(x));
  data.frame(num = c(mat[lower.tri(mat)]),
             den = c(t(mat)[lower.tri(mat)]))
}


#=================================================================================================================
#
# accuracy calculation for multi-level prediction
#
#=================================================================================================================

accuracy.multiLevel = function(observed, predicted, levels.obs = levels(as.factor(observed)))
{
  n = length(observed);
  if (length(predicted)!=n) stop("Lengths of 'observed' and 'predicted' must be the same.");

  t1 = table(factor(observed, levels = levels.obs));
  t2 = table(factor(predicted, levels = levels.obs));
  tt = table2.allLevels(observed, predicted, levels.x = levels.obs, levels.y = levels.obs);

  tt.naive = outer(as.numeric(t1), as.numeric(t2))/n

  acc.naive = sum(diag(tt.naive))/n;
  acc = sum(diag(tt))/n;

  acc.naive0 = max(t1)/n;

  list(accuracy = acc, accuracy.conditionalNaive = acc.naive, accuracy.naive = acc.naive0);
}
  
#=================================================================================================================
#
# numbers2colors with a better handling of low number of colors
#
#=================================================================================================================

numbers2colors2 = function(x, 
                     signed = NULL, 
                     centered = signed,
                     lim = NULL, 
                     commonLim = FALSE,
                     ...
                     naColor = "grey")
{
  ...
  xCol[!mmEq] = colors[round(xInd[!mmEq] * (nColors-1)) + 1];
