    ...
    widths2, heights2)
{
  ...
  if (sum(minDists > 0) > 0)
  {
    minPenalty = min(penalty[minDists > 0]);
    which.max(minDists[penalty==minPenalty]);
  } else
    which.max(minDists);
}
...
potentialPositions = function(
  ...
  stretch = 0.05, # Stretch dimensions by this fraction: 
                    ## this is done to prevent the rest of the algorithm from overlapping a label with another point at
                    ## the same coordinates as the point for that label.
  step.x, step.y,
  adj = c(0.5, 0.5),
  ...
  boxSize = NULL)
{
  ...
  {
    box = par("usr")
    boxSize = box[2]-box[1] + box[4]-box[3];
  }
  for (ds in 0:allowDistantLabels)
  {
    ...
    penalty = c(penalty, rep(boxSize*ds, 2*(nx + ny-2)));
  }
   ...
   ...)
{
  ...
  while (!done)
  {
    ...
    while (run < maxTries && !done)
    {
      ...
      while (ip <= nTestPoints && !done)
      {
        ...
        i = maxDistIndex(
           ...
           penalty = testPositions[[p]]$penalty,
    ...
    if (!done)
    {
      ...
      labW = labW[-remove];
      labH = labH[-remove];
      labelDists = labelDists[-remove, -remove, drop = FALSE];
      p.x = p.x[-remove];
      p.y = p.y[-remove];
...
#=======================================================================================================================
#
# Label groups of points
#
#=======================================================================================================================

# Produce a label for one point from each group.

labelGroupsOfPoints = function(
   x, y, labels,
   group = labels,
   cex = 1, font = 1,
   pt.cex = 1, ptExtension = 1.1,
...
{
  nPoints = length(x);
  candidates = labelPoints2(
    x = x, 
    y = y,
    labels = labels,
    cex = cex,
    font = font,
    pt.cex = pt.cex, ptExtension = ptExtension,
    col = col, xpd = xpd,
    protectEdges = protectEdges, 
    doPlot = FALSE,
    ratio.pointToChar = ratio.pointToChar, ratio.pointToChar.width = ratio.pointToChar.width,
    ratio.pointToChar.height = ratio.pointToChar.height,
    testStep.x = testStep.x, testStep.y = testStep.y,
    labelPreference = labelPreference,
    allowDistantLabels = allowDistantLabels,
    maxTries = maxTries,
    ratio.InToOutDist = 1.5,
    verbose = verbose, indent = indent, ...);

  if (sample > length(x)) sample = length(x);
  if (sample > 0 & sample < length(x))
  {
    ...
    sampled = sample(length(x), sample)
    x = x[sampled];
    y = y[sampled];
    labels = labels[sampled];
    groups = groups[sampled];
    ## FIXME: this will need more work if cex, font etc. are vectors
  }

  if (any( candidates$plotLabels))
  {
    cIndex = which(candidates$plotLabels);
    cGroup = group[cIndex];
    groupLevels = unique(cGroup);
    names(groupLevels) = groupLevels;
    xc = x[cIndex];
    yc = y[cIndex];
    mdst = mdist(cbind(xc, yc))
    diag(mdst) = NA;
    inGroupDist = lapply(groupLevels, function(gr) colMeans(mdst[ gr==cGroup, gr==cGroup], na.rm = TRUE));
    outGroupDist = lapply(groupLevels, function(gr) colMins(mdst[ gr!=cGroup, gr==cGroup], na.rm = TRUE));
    selectionStat = mymapply(function(ind, outd) ratio.InToOutDist * ind - outd, inGroupDist, outGroupDist);
    labelInd = sapply(groupLevels, function(gr) which(gr==cGroup)[ which.min(selectionStat) ]);
    p.x = candidates$x[labelInd];
    p.y = candidates$y[labelInd];
    if (doPlot)
      text(p.x, p.y, labels[cIndex][labelInd], col = col, cex = cex, xpd = xpd, adj = c(0.5, 0.5), font = font, ...)
    plotted = rep(FALSE, nPoints);
    plotted[which(candidates$plotLabels)[labelInd]] = TRUE;
    out = list(x = p.x, y = p.y, label = labels[cIndex][labelInd], 
             plotLabels = plotted, candidates = candidates );
  } else
    out = list(x = numeric(0), y = numeric(0), label = character(0),
             plotLabels = rep(FALSE, nPoints), candidates = candidates );
