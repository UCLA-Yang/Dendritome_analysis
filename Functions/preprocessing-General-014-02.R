...
# Version 014-02:
#   . Fixing an important bug where EBLM adjustment was skipped even though latent factors were non-null
#   . Adding the possibility of supplying explicit normalization factors to DESeq2, e.g. when using housekeeping genes or
#     spike-ins for normalization
#   . Making phenoColors optional
#   . geneAnnotation.he is by default also saved into a gzipped csv file.
#   . geneAnnot.[expr,xdata].OSR.collapsed is added to output component collapse
...
)
{
  ...
  if (length(removedCovariates)==0 && length(PCs)==0) return(x);
...
preprocessGeneral = function(
   ...
   # Optional arguments for size factor calculation in DESeq2
   sizeFactorArgs = list(type = "ratio", locfunc = stats::median),
   ...
   geneAnnotationFile.he = spaste(fileNamePrefix, "geneAnnotation.he.csv.gz"),
...
)
{
                     ...
                     pointLabels = NULL)
   {
    ...
    main1 = spaste(pca.mainBase, pca.mainSep, sub, if (pca.mainSep=="\n") "" else "               ");
    ...
    if (is.null(outlierIndicator)) outlierIndicator = rep(1, nrow(data));
    showOutlierLegend = length(unique(outlierIndicator)) >= 2
    for (cc in 1:nPlots)
    {
       ...
       plotPCA.2panels(
          ...
          borderFactor = if (showOutlierLegend)
               factor(c("Non-outlier", "Outlier")[outlierIndicator], levels = c("Non-outlier", "Outlier")) else NULL,
   ...
   while (changed)
   {
      ...
      if (length(HE.VST.OR[[ORStep]]$OSR)==0 || forceCalculation)
      {
         ...
         if (length(phenoColors) > 0) phenoColors.OSR$colors = phenoColors.OSR$colors[!outliers, , drop = FALSE];
         ...
         if (!any(outliers))
         {
           ...
           {
              if (verbose > 2) printFlush("Adjusting data after outlier removal...");
      ...
      colors.x$colors = if (length(phenoColors) > 0) cbind(phenoColors$colors, Outlier = outliers+1) else 
                             data.frame(Outlier = outliers+1);
   ...
   if (saveOSR)
   {
     ...
     write.csv.nr(geneAnnotation.he, quote = TRUE,
               file = file.path(sampleDir.OSR, geneAnnotationFile.he));
   ...
   if ((length(cl.probe2gene) > 0) && (forceCalculation || length(intermediateResults$collapse)==0))
   {
     ...
     if (saveCollapsed)
     {
       ...
       if ("OSR" %in% names(collapsedXData))
       {
         ...
         intermediateResults$collapse$geneAnnot.xdata.OSR.collapsed = geneAnnot.xdata.OSR.collapsed;
       }
       if ("OSR" %in% names(collapsedExpr))
       {
         ...
         intermediateResults$collapse$geneAnnot.expr.OSR.collapsed = geneAnnot.expr.OSR.collapsed;
       }
