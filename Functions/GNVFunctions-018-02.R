...
# 18: 
#     . GNV.rescueStatistics changed defaults for pert.bg.interName to match the standard prettified output of individual
#       analyses

# 18-02:
#     . Fixed point border in separate legend in plotPCA.2panels
                          ...
                          prettifyList = NULL)
{
   ...
   if (!is.null(prettifyList)) 
      out$testName.pretty = prettifyStrings(out$testName.pretty, prettifyList);
   out;
}
...
#### FIXME: the call to geneSetFnc contains topThresholds which are blithely ignored by this implementation; instead I try
###to get top thesholds by looking at the names of rescue/exacerbation columns in the results.
...
rescueGeneLists = function(
   ...
   orderSign = c(0, -1, 1),
   topThreshold1 = NULL,
   thresholdNamePattern= "",
   separateByThreshold = FALSE,
   ...)
{
  cols1 = grep(spaste(sourceCols[1], thresholdNamePattern), names(results));
  ...
  out = lapply(suffixes, function(suf)
  {
    unlist(removeNames(mymapply(function(colName, orderColName, orderSign)
    {
      ...
      if (is.na(colInd1)) stop("Could not find column ", colName1, " among columns of 'results'.");
      ind = results[, colInd1];
...
#======================================================================================================
#
# extended versions of addRescueColumns and rescueGeneLists when there are combinations of rescue perturbations that can
# enhance or reverse one another
#
#======================================================================================================

addRescueColumns.ext = function(
    normalizedData,
    dataIsLogTransformed = FALSE,
    logBase = 2,
    logShift = 1,
    ...
    #testNames,
    #combName,
    wt = "WT", background="Q175", perturbations,
    pertBack = spaste(background, "/", perturbations),
    rescueColNameExtension = spaste(perturbations, ".in.", background),
    rescueColSep = testNameSep,
    enhancementCombinations = list(),  ## Should be a list of pairs of elements in rescueColNameExtension
    enhancementCombinationNames = names(enhancementCombinations),
    levelSep = ".vs.",
    testNameSep = ".for.",
    pattern.Z = spaste("Z", testNameSep),
    pattern.p = spaste("p", testNameSep),
    ...
    ...)
{
  # Add rescue columns for both Homo and Het KO

  nPerturbations = length(perturbations);
  if (length(pertBack)!= nPerturbations) stop("Length of 'pertBack' must equal length of 'perturbations'.");
  if (length(background)==1) background = rep(background, nPerturbations)
  if (length(wt)==1) wt = rep(wt, nPerturbations)

  for (pr in length(perturbations):1)
    res = addRescueColumns(
          ...
          pheno = pheno,
          res = res,
          wt = wt[pr], background = background[pr],
          perturbation = pert[pr],
          pertBack = pertBack[pr],
          levelSep = levelSep,
          testNameSep = testNameSep,
          pattern.Z = pattern.Z,
          pattern.p = pattern.p,
          pattern.FDR = pattern.FDR,
          pattern.mean = pattern.mean,
          pattern.LFC = pattern.LFC,
          stopOnMultipleMatches = stopOnMultipleMatches,
          genotypeColumnName = genotypeColumnName,
          topThresholds = topThresholds,
          topThresholdNames = topThresholdNames,
          baselineLog2FC = baselineLog2FC,
          baselineZ = baselineZ,
          baselineP = baselineP,
          baselineFDR = baselineFDR,
          rescueColNameExtension = make.names(spaste(rescueColSep, rescueColNameExtension[pr])),
          ...);

  if (length(enhancementCombinations)==1 && length(enhancementCombinationNames==0)) enhancementCombinationNames = "";
  combinedRescue = mymapply(function(ec, ecName)
  {
    scoreCols = spaste("rescueScore", rescueColSep, ec);
    # Enhancement score: minimum of the rescue scores (i.e., minimum of the Z statistis that go into the rescue scores),
    # positive if the rescue scores have the same sign
    sameSign = c(-1, 1) [ 1 + (abs(rowSums(sign(as.matrix(res[, scoreCols]))))==length(scoreCols))]
    enhancementScore = sameSign * do.call(pmin, abs(res[scoreCols]));
    out = do.call(cbind, c(list(XXXX = enhancementScore), removeListNames(lapply(topThresholdNames, function(tname)
    {
      # There is a move, rescue and exacerbation column for each threshold
      movedCols = spaste("significantMove", rescueColSep, ec, tname);
      rescueCols = spaste("significantRescue", rescueColSep, ec, tname)
      exacCols = spaste("significantExacerbation", rescueColSep, ec, tname)

      significantEnhancement = enhancementScore > 0 & rowSums(as.matrix(res[, movedCols]))==length(movedCols);
      significantReversal =  enhancementScore < 0 & rowSums(as.matrix(res[, movedCols]))==length(movedCols)
      out = cbind(significantModification = as.numeric(significantEnhancement | significantReversal),
                  significantEnhancement = 0 + significantEnhancement,
                  significantReversal = 0 + significantReversal);
      setColnames(out, spaste(colnames(out), prependPrefix(".", ecName), tname));
    }))));
    setColnames(out, sub("XXXX", spaste("enhancementScore", prependPrefix(".", ecName)), colnames(out)));
  }, enhancementCombinations, enhancementCombinationNames);

  insertPosition = min(grep("meanExpr", names(res)));
  out = do.call(
          cbind, 
           c(list(res[, c(1:(insertPosition-1))]),
           removeListNames(combinedRescue), 
           list(res[, c(insertPosition:ncol(res))]))); 
  out;
}

rescueGeneLists.ext = function(
   results,
   topList.minN = 0,
   geneIDs,
   backupGeneIDs = NULL,
   wt = "WT", background="Q175", perturbations,
   pertBack = spaste(background, "/", perturbations),
   rescueColNameExtensions = spaste(perturbations, ".in.", background),
   rescueColSep = testNameSep,
   enhancementCombinations = list(),  ## Should be a list of pairs of elements in rescueColNameExtension
   enhancementCombinationNames = names(enhancementCombinations),
   pattern.Z = spaste("Z", testNameSep),
   levelSep = ".vs.",
   signColName = spaste(pattern.Z, make.names(background), levelSep, make.names(wt)),
   signColName.enhancement = spaste(pattern.Z, make.names(background[1]), levelSep, make.names(wt[1])),
   testNameSep = ".for.",
   andSep = ".and.",
   nameModifier = spaste(make.names(background[1]), levelSep, make.names(wt[1])),
   nameModifier.enhancement = rep(spaste(make.names(background[1]), levelSep, make.names(wt[1])), 
                                     length(enhancementCombinations)),
   thresholdNamePattern = "\\._",
   separateByThreshold = FALSE,
   ...)
{
  geneIDs = combineMainAndBackupIDs(geneIDs, backupGeneIDs);

  background = make.names(background);
  wt = make.names(wt);
  perturbations = make.names(perturbations);
  pertBack = make.names(pertBack);
  signColName.enhancement = make.names(signColName.enhancement);
  signColName = make.names(signColName);
  rescueColNameExtensions = make.names(rescueColNameExtensions);

  nPerturbations = length(perturbations);
  if (length(pertBack)!= nPerturbations) stop("Length of 'pertBack' must equal length of 'perturbations'.");
  if (length(background)==1) background = rep(background, nPerturbations)
  if (length(wt)==1) wt = rep(wt, nPerturbations)
  if (length(signColName)==1)  signColName = rep(signColName, nPerturbations);


  sourceCols0 = c("significantMove", "significantRescue", "significantExacerbation");
  enhancementCols0 = c("significantModification", "significantReversal", "significantEnhancement");
  #perturbationExtensions = spaste(testNameSep, c(perturbation, perturbation2, perturbation2),
  #                                                   ".in.", c(background, background, perturbation));
  perturbationExtensions = spaste(rescueColSep, rescueColNameExtensions);

  nEnhancementScores = length(enhancementCombinationNames);
  if (length(signColName.enhancement)==1) signColName.enhancement = rep(signColName.enhancement, nEnhancementScores);

  lists1 = lapply(1:nPerturbations, function(pr)
     rescueGeneLists(
        wt = wt[pr], background = background[pr], perturbation = perturbations[pr],
        pertBack = pertBack[pr],
        signColName = signColName[pr],
        sourceCols = spaste(sourceCols0, perturbationExtensions[pr]),  
        orderCols = spaste(rep("rescueScore", length(sourceCols0)), perturbationExtensions[pr]),
        ...
        testNameSep = testNameSep,
        andSep = andSep,
        nameModifier = nameModifier[pr],
        thresholdNamePattern = thresholdNamePattern,
        separateByThreshold = separateByThreshold,
        ...));

  lists2 = lapply(1:nEnhancementScores, function(ec)
     rescueGeneLists(
        signColName = signColName.enhancement[ec],
        sourceCols = spaste(enhancementCols0, prependPrefix(".", enhancementCombinationNames[ec])),
        orderCols = rep(spaste("enhancementScore", prependPrefix(".", enhancementCombinationNames[ec])), 
                        length(enhancementCols0)),
        orderSign = c(0, 1, -1),
        results = results,
        topList.minN = topList.minN,
        geneIDs = geneIDs,
        ...
        testNameSep = testNameSep,
        andSep = andSep,
        nameModifier = nameModifier.enhancement[ec],
        thresholdNamePattern = thresholdNamePattern,
        separateByThreshold = separateByThreshold,
        ...));
  ...
  if (separateByThreshold)
  {
    lst = c(lists1, lists2);
    nt = length(lst[[1]]);
    lapply(1:nt, function(tt) unlist(lapply(lst, `[[`, tt), recursive = FALSE));
  } else 
     unlist(removeListNames(c(lists1, lists2)), recursive = FALSE)
}
...
GNV.addCircadianWGCNAcollection = function(
     ...
     file.WGCNA = NULL,
     file.DE = NULL,
     DEThreshold = c("._FDR.lessThan.0.05_.", "._FDR.lessThan.0.05.and.FC.greaterThan.0.1_.",
   "._FDR.lessThan.0.05.and.FC.greaterThan.0.2_.", "._FDR.lessThan.0.05.and.FC.greaterThan.0.3_."),
     WGCNAName = "CircadianWGCNA",
     collections = NULL, 
     addCombined = TRUE, combinedName = "AllCombined")
{
  ...
  DEThreshold = match.arg(DEThreshold);
  ...
  if (is.null(file.DE))
    file.DE = spaste(getRootDir(),
          ...
          "collectionOfDEGenes-CircadianExpression.", availableTissues[tsIndex], "-",
          DEThreshold, ".RData");
  ...
  collections = c(collections,
        list(xxxxx219843928492 = convertCollectionToOrganism(coll, organism = organism)));
  names(collections) = sub("^xxxxx219843928492", WGCNAName, names(collections));
...
{   
  ...
  collections1 = rearrangeInternalCollections(
     ...
     separateSCHttSets = separateSCHttSets, SCHttTags = SCHttTags);
...
plotPCA.2panels = function(
  ...
  borderFactor = NULL,
  pointLabels = NULL,
  borderAndColorAreIndependent = TRUE,
  ...
  ...)
{
  ...
  if (separateLegend && plotLegend && !is.null(colorFactor))
  {
    ...
    if (!is.null(borderFactor))
    {
      ...
      borderAndColorAreIndependent2 = (nBorderLevels > 1) && borderAndColorAreIndependent;
      if (any(is.na(suppressWarnings(as.numeric(borderLevels)))))
      {
        borderColors = labels2colors(as.numeric(as.factor(borderFactor)), colorSeq = borderColorSequence);
        borderColorLevels = labels2colors(1:nBorderLevels, colorSeq = borderColorSequence);
      } else {
        borderColors = as.numeric(as.character(borderFactor));
        borderColorLevels = sort(unique(borderColors));
      }
      if (is.numeric(borderColorSequence)) borderColorLevels = as.numeric(borderColorLevels);
    } else {
      borderColors = 1
      borderColorLevels = 1;
      borderLevels = "";
      nBorderLevels = 1;
      borderAndColorAreIndependent2 = FALSE;
    }
    if (!is.null(prettifyList))
    {
      ...
      borderLevels = prettifyStrings(borderLevels, prettifyList);
    }
    nShapeLevels = length(shapeLevels)
    par(mar = c(mar[1], 0.2, mar[3], 0.2));
    par(cex = fig.cex);
    plot(c(0,1), type = "n", axes = FALSE, frame = FALSE, xlab = "", ylab = "")
    bg = labels2colors(c(1:nCFLevels, rep(0, nShapeLevels + borderAndColorAreIndependent2 * nBorderLevels)), 
                       colorSeq = colorSequence)
    col = c(rep(if (borderAndColorAreIndependent2) borderColorLevels[1] else borderColorLevels, nCFLevels), 
            if (borderAndColorAreIndependent2) borderColorLevels else NULL, rep(borderColorLevels[1], nShapeLevels))
    out = legendClean("top",
              legend = c(spaste(colorPrefix, cfLevels), 
                         if (borderAndColorAreIndependent2) spaste(borderPrefix, borderLevels) else NULL, 
                         spaste(rep(shapePrefix, nShapeLevels), shapeLevels)),
              ncol = legendNCols,
              pt.cex = min(pt.cex, 1.5),
              pch = c(rep(firstShape, nCFLevels + borderAndColorAreIndependent2 * nBorderLevels), 
                      firstShape - 1+seq_len(nShapeLevels)),
              pt.bg = bg,
              col = col,
              pt.lwd = lwd);
  ...
  plotPCA(data1, genotype = colorFactor,
          ...
          borderColorSequence = borderColorSequence,
...
plotPCA = function(
  ...
  device = "pdf",
  plotLegend = TRUE,
  legendNCols = 1,   ### FIXME: for now this is ignored, legend automatically tries 1 to 4 columns.
  prettifyList = NULL,
  includePVE = TRUE,
  ...)
{
  ...
  if (!is.null(borderFactor))
  {
      ...
      if (is.numeric(borderColorSequence)) borderColors = as.numeric(borderColors);
  } else
    borderColors = 1;
   ...
   maxAddTopGenes = 20)
{
  ...
  char.top = lapply(analysis$topSymbols.extraLists[[1]],
         function(x) if (length(x) > 0) spaste(" (",
              paste(x[1:nAdd(length(x))], collapse = ", "),  if (length(x) > maxAddTopGenes) ", ..." else "", ")") else "");
...
GNV.rescuePlotAttributes = function(analysis,
   ...
   baseBgLegend = NULL)
{
  statName = make.names(spaste("Z.for.", pertBg, ".vs.", bg))

  thresholdType = match.arg(thresholdType);
  sigName = make.names(spaste(thresholdType, ".for.", pertBg, ".vs.", bg))
  ...
  {
    change = !is.na(baseBg);
    pointBg[change] = baseBg[change];
    class.num[change] = class.num[change] + 9;
  }
  ...
  list(pointColor = pointColor, pointBg = pointBg, pointOrder = order,
       ...
       baseBg = baseBg,
       baseBgLevels = baseBgLevels,
       baseBgLegend = baseBgLegend)
}
...
GNV.addRescueLegend = function(plotAttr, ...)
{
  ...
  legendClean(..., pch = 21, col = c(plotAttr$rescueColors[keepTypes], rep("grey", length(plotAttr$baseBgLevels))),
              pt.bg = c(plotAttr$rescueBgColors[keepTypes], plotAttr$baseBgLevels),
              legend = c(names(plotAttr$rescueColors)[keepTypes], plotAttr$baseBgLegend));
}
  ...
  ...)
{
  ...
  symbol = data[[labelColumn]];

  if (length(excludeIndex)>0) if (is.logical(excludeIndex)) excludeIndex = which(excludeIndex);

  annotatedScatterplot(x, y, verbose = TRUE, 
              xlab = prettifyStrings(col1, mymapply("c", analysis$prettifyList.plots, prettifyList)),
              ylab = ylab,
              excludeIndex = excludeIndex,
              pointLabels = replaceMissing(data[[labelColumn]]),
              main = spaste(main, "\n"),
              cex.lab = cex.lab, cex.axis = cex.axis,
              col = col, bg = bg, pch = pch, pt.cex = pt.cex,
              showPoints = showPoints,
              xlim = xlim, ylim = ylim,
              stretchLims = stretchLims,
              ...
              mustShow.signifOnly = mustShow.signifOnly,
              cex.sideLabels = cex.sideLabels,

              ab.thresholdType= ab.thresholdType,
              ab.threshold = ab.threshold,
              fdr.x = fdr.x,
              fdr.y = fdr.y,
              addAblines = addAblines,
              addAxisLines = addAxisLines,
              addLegend = addLegend,
              pointAttr = plotAttr,
              legendPos = legendPos, 
              leg.pt.cex = leg.pt.cex,
              leg.cex = leg.cex,
              addCounts = addCounts,
              addOverlapPValues = addOverlapPValues, ...)
}
  ...
  ...)
{
  ...
  {
    excludeIndex = match(excludeSymbols, res[[labelColumn]]);
    if (any(is.na(excludeIndex)))
    {
      warning("Could not find the following entries in 'excludeSymbols' in 'res$", labelColumn, "':\n  ",
  ...
  plotData = rescuePlot.base(
       ...
       nodeNames = res[[labelColumn]], plotLegend = FALSE,
  ...
  if (!is.null(summaryModuleRescue))
  {
    ...
    width = pmin(nGenesInModules/2, 30 + nGenesInModules/5);
...
)
{
  ...
  mean = try( {tapply(rescueScore, moduleLabels, mean, na.rm = TRUE);})
  ...
  ...)
{
  ...
  data = extendedResults(analysis)[[1]];
  if (!labelColumn %in% names(data)) warning(.immediate = TRUE, 
     "GNV.compareWithExternalData: 'labelColumn' (value: ", labelColumn, ") is not among names of results.");
  if (!is.null(plotDevice))
  {
    device = match.fun(plotDevice);
    device(file = file.path(plotDir,
  ...
  annotatedScatterplot(x, y, verbose = TRUE,
              ...
              pointLabels = data[[labelColumn]],
              nLabel = nLabel,
              nConsider = nConsider,
              cex.labels = cex.labels,
              forceLabel = mustShowLabels,
              labelDirections = labelDirections,
              scaleForSelection = TRUE,
              mustShowLabels = mustShowLabels,
              mustShow.signifOnly = mustShow.signifOnly,
...
GNV.standardPlots = function(
   ...
   nSignif.cex = 1,
   nSignif.main = spaste("Numbers of significantly associated genes, ", analysisName.pretty),
   ...
   nMoved.main = spaste("Numbers of rescued or exacerbated genes, ", analysisName.pretty, "\n",
   ...
   rsc.mustShowLabels = NULL,
   rsc.mustShow.signifOnly = FALSE,
   ...
   eds.mustShowLabels = NULL,
   eds.mustShow.signifOnly = FALSE,
   eds.cex.sideLabels = 1,
   ...
   )
{
  ...
  if (!is.null(eds.externalData) || !is.null(eds.externalDataCol))
  {
     ...
     for (pl in 1:nPlots)
       GNV.compareWithExternalData(
          ...
          labelColumn = geneSymbolColumn,
          mustShowLabels = eds.mustShowLabels,
          mustShow.signifOnly = eds.mustShow.signifOnly,
          cex.sideLabels = eds.cex.sideLabels,
  ...
  GNV.rescueScatterplot(
     ...
     labelColumn = geneSymbolColumn,
     mustShowLabels = rsc.mustShowLabels,
     mustShow.signifOnly = rsc.mustShow.signifOnly,
     cex.sideLabels = rsc.cex.sideLabels,
     cex.labels = rsc.cex.labels,
     cex.lab = rsc.cex.lab,
     cex.axis = rsc.cex.axis,
     #cex.main = rsc.cex.main,
     col = rsc.col,
     bg = rsc.bg,
     pch = rsc.pch,
     pt.cex = rsc.pt.cex,
  ...
  GNV.rescueScatterplot.FoldChanges(analysis, analysisName = analysisName,
     wt = wt, background = background, perturbation = perturbation, pertBack = pertBack,
     LFCpattern = "log2FC",
  ...
  if (plotNoColor) GNV.rescueScatterplot.FoldChanges(analysis, analysisName = analysisName,
     wt = wt, background = background, perturbation = perturbation, pertBack = pertBack,
     LFCpattern = "log2FC",
     baselineFCCol = rsc.baselineFCCol,
     reverseSign = rsc.reverseSign,
...
GNV.rescueStatistics = function(results, background, perturbation, 
    ...
    pert.bg.interName = spaste("interaction of ", perturbation, " KO with ", background),
...
GNV.newPreprocessingInfo = function(
  perturbation,
  gene = perturbation,
  perturbation.pretty = spaste(perturbation, " HET KO"),
  year, 
  projectID,
  organism = "mouse",
  background = "Q140",
  ...
  outputBaseDir = spaste(outputBaseDir.all, year, "-", projectID, "-",
                                     perturbation, ".x.", background, ".", age, "m.", tissue, prependPrefix("-", nameModifier)),
  ...
  setName = spaste(perturbation, ".", tissue, ".", age, "m", prependPrefix("-", nameModifier)),
  setName.pretty = spaste(perturbation.pretty, " x ", background, ", ", age, " month ", tolower(tissue),
                          nameModifier.pretty),
  amNameModifier = "",
  amNameSource = spaste("Nan Wang/Yang lab "),
  amAnalysisName = spaste(perturbation.pretty, " in ", background, 
  ...
  amAnalysisDescription = spaste("Bulk RNA-seq from ", age, " month old mouse ", tolower(tissue),
             ", ", perturbation.pretty, " x ", background, " cross and controls."),
  amDataSource = "Data from Nan Wang/Yang lab and UCLA sequencing core.",
  amPreprocessingComponent = "preprocessing",
  amAnalysisComponent = "individual analysis",
  ...
  covariates = character(0))
{
  ...
  out = list(perturbation = perturbation,
       gene = gene,
       perturbation.pretty = perturbation.pretty,
       year = year,
       projectID = projectID,
       organism = organism,
       background = background,
      ...
      # sampleNamePattern = sampleNamePattern,
       ...
       amPreprocessingComponent = amPreprocessingComponent,
       amAnalysisComponent = amAnalysisComponent,
...
{      
 ...
 dataList = with(preprocessingInfo,
 {
  ...
  sampleAnnot0[[sampleAnnot.genotypeCol]] = gsub(" ", "", sampleAnnot0[[sampleAnnot.genotypeCol]]);
  if (!is.null(preprocessingInfo$genotypeTranslation))
    sampleAnnot0[[sampleAnnot.genotypeCol]] = 
      translateUsingTable( sampleAnnot0[[sampleAnnot.genotypeCol]], genotypeTranslation, keepUntranslated = FALSE);

  keepSamples = !is.na(sampleAnnot0[[sampleAnnot.genotypeCol]]) & 
                    !sampleAnnot0[[sampleAnnot.sampleCol]] %in% manuallyExcludedSamples
  sampleAnnot02 = sampleAnnot0[keepSamples, ];

  sampleAnnot02[[sampleAnnot.genotypeCol]] = sub(" +$", "", sampleAnnot02[[sampleAnnot.genotypeCol]]);
  sampleAnnot02[[sampleAnnot.genderCol]] = sub(" +$", "", sampleAnnot02[[sampleAnnot.genderCol]]);
...
#=======================================================================================================
#
# GNV.newCrossAnalysisInfo and newCrossAnalysisData
#
#=======================================================================================================

# These functions use information in lists filled out by GNV.newPreprocessingInfo to fill in sample annotation, designInfo
# and everything else needed for the analysis, except counts, weights, geneAnnotation and rescue arguments for the Info
# function. If there are enough samples of both sexes, sex-specific subsets are also created.

GNV.newCrossAnalysisInfo = function(ci)
{
  printFlush(spaste("Working on perturbation ", ci$perturbation));
  sampleAnnot0 = om.retrieveRObject(om.getObject(nameVector = c(ci$amAnalysisName, ci$amPreprocessingComponent,
               "OR sample information")));

  sampleAnnot1 = binarizeCategoricalColumns.pairwise(
        sampleAnnot0[, c(ci$sampleAnnot.genotypeCol, ci$sampleAnnot.genderCol)],
        levelOrder = list(c("WT", ci$gene, ci$background, spaste(ci$background, "/", ci$gene)),
                         character(0)), checkNames = TRUE, includePrefix = FALSE);
  sa2 = data.frame(Q140 = as.numeric(grepl(ci$background, sampleAnnot0[[ci$sampleAnnot.genotypeCol]])),
                   KO = as.numeric(grepl(ci$gene, sampleAnnot0[[ci$sampleAnnot.genotypeCol]])));
  names(sa2) = c(ci$background, ci$gene);
  sampleAnnot2 = centerBinaryColumns(cbind(sampleAnnot0, sampleAnnot1, sa2), center = FALSE);
  print(table(sampleAnnot2[[ci$sampleAnnot.genderCol]], sampleAnnot2[[ci$sampleAnnot.genotypeCol]]));

  sex = sampleAnnot0[[ci$sampleAnnot.genderCol]];
  if (length(unique(sex))>1)
  {
    sampleAnnot.byGender = splitData(sampleAnnot2, split = sex, nameBase = spaste(ci$setName, "."));
  
    setNames.byGender = names(sampleAnnot.byGender);
    setNames.pretty.byGender = rep(ci$setName.pretty, each = 2)
    setNames.pretty.byGender = spaste(setNames.pretty.byGender, c("F", "M"));
    setNames.pretty.byGender = multiSub(c("striatumF", "striatumM", "cortexF", "cortexM"),
                 c("female striatum", "male striatum", "female cortex", "male cortex"), setNames.pretty.byGender,
                   ignore.case = TRUE);

    amAnalysisNames.byGender = rep(ci$amAnalysisName, each = 2)
    amAnalysisNames.byGender = mapply(function(n, s) multiSub(ci$tissue, paste(s, tolower(ci$tissue)), n, ignore.case = TRUE), 
                                   amAnalysisNames.byGender, c("female", "male"));
    genderSpecific3 = c(FALSE, TRUE, TRUE);
  } else {
    sampleAnnot.byGender = NULL;
    setNames.byGender = setNames.pretty.byGender = amAnalysisNames.byGender = NULL;
    genderSpecific3 = FALSE;
  }

  multiSampleAnnot = c(multiData(sampleAnnot2), sampleAnnot.byGender);

  # Check that all genes are good for analysis. In some data sets there are genes that are ok in all samples but not when
  # restricted to either sex.

  nSets1 = length(multiSampleAnnot);
  amAnalysisNames3 = c(ci$amAnalysisName, amAnalysisNames.byGender);
  setNames3 = c(ci$setName, setNames.byGender);
  setNames.pretty3 = c(ci$setName.pretty, setNames.pretty.byGender);
  setNames.short3 = sub(spaste(ci$perturbation, "."), "", setNames3);
  setNames.prettyShort3 = capitalize(sub(spaste(ci$perturbation.pretty, " x Q140, "), "", setNames.pretty3))

  names(multiSampleAnnot) = setNames3;

  covariates = ci$covariates;
  if (ci$sva.useNFactors > 0) covariates = c(covariates, spaste("SVAFactor.", 1:ci$sva.useNFactors));

  designInfo1 = GNV.createDesignsAndContrasts(
      background = ci$background, perturbation =ci$gene,
      genotypeName = ci$sampleAnnot.genotypeCol,
      covariates = covariates,
      combineSignificantHits = TRUE,
      all = TRUE);

  interactionIndex = 1:2;  # perturbation vs. WT and Q140/pert vs. Q140

  designs.int = spaste(designInfo1$testName[interactionIndex], "*M.vs.F");
  ndi = length(designs.int);
  designInfo.si0 = data.frame(
     design = spaste("~ ", designs.int, prepChar(paste(covariates, collapse = " + "), " +")),
     contrast.name = rep(NA, ndi),
     contrast.numerator = rep(NA, ndi),
     contrast.denominator= rep(NA, ndi),
     coefficientName = sub("*", ":", designs.int, fixed = TRUE),
     testName = spaste("inter.", designInfo1$testName[interactionIndex], ".with.Sex"),
     testName.pretty = spaste("interaction of ", designInfo1$testName.pretty[interactionIndex], 
                          " with sex"),
     combineSignificantHits = rep(TRUE, ndi),
     splitSignificantHitsBy = designInfo1$testName[interactionIndex]);

  coeff.pq = spaste(ci$background, "*", ci$gene);

  npq = length(coeff.pq);

  designInfo.pq = data.frame(
     design = spaste("~ ", coeff.pq, prepChar(paste(covariates, collapse = " + "), " +")),
     contrast.name = rep(NA, npq),
     contrast.numerator = rep(NA, npq),
     contrast.denominator= rep(NA, npq),
     coefficientName = sub("*", ":", coeff.pq, fixed = TRUE),
     testName = spaste("inter.", ci$gene, ".with.", ci$background),
     testName.pretty = spaste("interaction of ", ci$gene, " with ", ci$background),
     combineSignificantHits = TRUE,
     splitSignificantHitsBy = spaste(ci$background, ".vs.WT"));

  designInfo = listRep(rbind(designInfo1, designInfo.pq), nSets1);
  if (length(unique(sex))>1) designInfo[[1]] = rbind(designInfo[[1]], designInfo.si0);

  # Shortcut... the selected are always the first 4 rows of all.
  designInfo.selected = lapply(designInfo, function(x) x[1:4, ]);

  combineResults = mymapply(function(x, y) 
  {
    out = list(x$testName, y$testName);
    names(out) = c("all", "core GNV");
    out;
  }, designInfo, designInfo.selected)

  combineNames.pretty = lapply(combineResults, function(lst)
  {
    multiSub(spaste(ci$setName, "."), spaste(ci$setName.pretty, ", "), names(lst), fixed = TRUE)
  })

  combineColNameExtensions = lapply(combineResults, function(x) rep("", length(x)));

  enrichment.dropTests = mymapply(function(desInfo1)
  {
     ind = gregexpr(ci$background, desInfo1$testName, fixed = TRUE);
     ind = lapply(ind, function(i1) if (length(i1)==1 && i1==-1) numeric(0) else i1);
     desInfo1$testName[sapply(ind, length)==1];
  }, designInfo)

  collection.keepTests = mymapply(function(desInfo1, bg)
  {
     ind = gregexpr(bg, desInfo1$testName, fixed = TRUE);
     ind = lapply(ind, function(i1) if (length(i1)==1 && i1==-1) numeric(0) else i1);
     desInfo1$testName[sapply(ind, length)!=1 & !grepl("inter", desInfo1$testName)];
  }, designInfo, MoreArgs = list(bg = ci$background))


  if (length(unique(sex))>1)
  {
    # Create AM analyses for the male and female data
    setGenders0 = c("female", "male");

    analysis = am.getAnalysis(am.analysisID(name = ci$amAnalysisName))
    description0 = analysis$description;
    descriptions.byGender = rep(description0, each = 2)
    descriptions.byGender = mapply(function(d, g) sub("mouse", spaste(g, " mouse"), d),
                                descriptions.byGender, setGenders0)

    parentSet = rep(ci$amAnalysisName, each = 2);

    ids = sapply(amAnalysisNames.byGender, am.analysisID);
    if (any(is.na(ids)))
       mymapply(am.insertNewAnalysis,
           name = amAnalysisNames.byGender[is.na(ids)],
           description = descriptions.byGender[is.na(ids)],
           shortName = setNames.byGender[is.na(ids)],
         MoreArgs = list(
           tissue = ci$tissue,
           metaData = list(derivedFromAnalysis = ci$amAnalysisName),
           organism = "mouse",
           dataSource = analysis$dataSource));
  } else setGenders0 = character(0);

  setGenders = c("", setGenders0);

  prettifyList = mymapply(c, prettifyList.general,
              list( spaste(ci$background, ".", ci$perturbation),
                    spaste(ci$background, "/", ci$perturbation)));

  baseGroupName = spaste("DE genes in ", ci$age, " month old ", 
                         tolower(ci$tissue), " of a ", ci$perturbation.pretty, " x ", 
                         ci$background, " mouse cross");
  baseGroups = list(newGroup(baseGroupName,
                  spaste("Differentially expressed genes between various genotype combinations in ",
                         ci$age, " month old ", 
                         tolower(ci$tissue), " of a ",
                         ci$perturbation.pretty, " x ", ci$background, " mouse cross."),
                  "Data by Nan Wang/Yang lab, analysis by Peter Langfelder",
                  parents = c(ci$tissue, "mRNA differential expression")));
  # Note: standard groups are added automatically within individualAssociation.general

  out = c(ci, list(
       amAnalysisNames = amAnalysisNames3,
       setNames = setNames3, 
       setNames.pretty = setNames.pretty3,
       setNames.short = setNames.short3,
       setNames.prettyShort = setNames.prettyShort3,
       genderSpecific = genderSpecific3,
       setGender = setGenders,
       multiSampleAnnot = multiSampleAnnot,
       designInfo = designInfo,
       designInfo.selected = designInfo.selected,
       combineResults = combineResults,
       combineNames.pretty = combineNames.pretty,
       combineColNameExtensions = combineColNameExtensions,
       collection.keepTests = collection.keepTests,
       enrichment.dropTests = enrichment.dropTests,
       baseGroupName = baseGroupName,
       baseGroups = baseGroups,
       internalClassification = spaste(ci$perturbation.pretty, " x ", ci$background),
       geneSource = "Data from Nan Wang/Yang lab, analysis by Peter Langfelder",
       prettifyList = prettifyList,
       sex = sex));
  out; 
}

GNV.newCrossAnalysisData = function(ci, refQvsWTRes, asNetwork, asData)
{
  printFlush(spaste("Working on ", ci$setNames[1]));
  if (is.null(ci$multiSampleAnnot)) ci = GNV.newCrossAnalysisInfo(ci);

  counts0 = om.retrieveRObject(om.getObject(nameVector = c(ci$amAnalysisName, ci$amPreprocessingComponent,
               "OR data for consensus individual analysis")))
  weights0 = om.retrieveRObject(om.getObject(nameVector = c(ci$amAnalysisName, ci$amPreprocessingComponent,
               "IO replacement weights for data for individual analysis")));

  weights = weights0[, colnames(counts0)];
  geneAnnot0 = om.retrieveRObject(om.getObject(
      nameVector = c(ci$amAnalysisName, ci$amPreprocessingComponent, "feature data for individual analysis")));
  geneAnnot = geneAnnot0[match(colnames(counts0), geneAnnot0[, 1]), ];

  if (length(ci$setNames) > 1)
  {
    sex = ci$multiSampleAnnot[[1]]$data[[ci$sampleAnnot.genderCol]];
    counts.byGender = splitData(counts0, split = sex, nameBase = spaste(ci$setName, "."));
    weights.byGender = splitData(weights, split = sex, nameBase = spaste(ci$setName, "."));
  } else {
    counts.byGender = weights.byGender = NULL;
  } 

  multiCounts = c(multiData(counts0), counts.byGender);
  multiWeights = c(multiData(weights), weights.byGender);

  nSets1 = length(ci$setNames);

  # I assume here that refQvsWTRes is ordered in the same gender order as multiCounts
  geneAnnot.ext = lapply(refQvsWTRes[1:nSets1], function(ref)
  {
    net = asNetwork[[ci$tissue]][match(geneAnnot$Entrez, asNetwork[[ci$tissue]]$Entrez), ]
    ga2ref = match(geneAnnot$Entrez, ref$Entrez);
    p = ref$`p for Q140 vs. WT`;
    pBonf = p.adjust(p, method = "bonferroni");
    oas = asData[ match(geneAnnot$Entrez, asData0$Entrez), ]
    out = data.frame(geneAnnot,
       module = net$module,
       ownModuleMembership = net$ownModuleMembershipZScore,
       log2FC.for.Q140.vs.WT.in.allGNV = ref$`log2FC for Q140 vs. WT`[ga2ref],
       Z.for.Q140.vs.WT.in.allGNV = ref$`Z for Q140 vs. WT`[ga2ref],
       p.for.Q140.vs.WT.in.allGNV = p[ga2ref],
       pBonferroni.for.Q140.vs.WT.in.allGNV = pBonf[ga2ref],
       FDR.for.Q140.vs.WT.in.allGNV = ref$`FDR for Q140 vs. WT`[ga2ref],
       log2FC.for.Q140.vs.Q20.in.AllelicSeries = oas$log2FoldChange.Q.140.vs.20,
       Z.for.Q140.vs.Q20.in.AllelicSeries = oas$stat.Q.140.vs.20,
       p.for.Q140.vs.Q20.in.AllelicSeries = oas$pvalue.Q.140.vs.20,
       FDR.for.Q140.vs.Q20.in.AllelicSeries = oas$FDR.Q.140.vs.20);
    out;
  });

  rescueInfoArgs = lapply(geneAnnot.ext, function(ga)
    list(wt = "WT", background = ci$background, perturbation = ci$gene,
       pertBack = spaste(ci$background, "/", ci$gene),
       genotypeColumnName = ci$sampleAnnot.genotypeCol,
       baselineZ = ga$Z.for.Q140.vs.WT.in.allGNV,
       baselineLog2FC = ga$log2FC.for.Q140.vs.WT.in.allGNV,
       baselineP = ga$p.for.Q140.vs.WT.in.allGNV,
    # take the bonferroni corrected p value as the baseline statistic to compare to FDR.
       baselineFDR = ga$pBonferroni.for.Q140.vs.WT.in.GNV));

  names(multiCounts) = names(multiWeights) = ci$setNames;

  c(ci, list(multiCounts = multiCounts, multiWeights = multiWeights,
       geneAnnot = geneAnnot.ext,
       rescueInfoArgs = rescueInfoArgs));
}
