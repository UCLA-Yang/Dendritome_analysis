...
intersampleDistanceConnectivity = function(x, blockSize = 5000)
{
  blocks = allocateJobs(nrow(x), ceil(nrow(x)/blockSize));
  if (length(blocks)==1) {
    ...
    for (bl in blocks)
    {
      dist = dist2(x, x[bl, ], setDimnames = FALSE);
      Z0[bl] = colSums(dist);
    }
  }
  Z0;
...
if (FALSE)
{
  ...
  Z1 = intersampleDistanceConnectivity(x)
  Z2 = intersampleDistanceConnectivity(x, blockSize = 100)
  all.equal(as.numeric(Z1), as.numeric(Z2))
}
...
iterativeOutlierRemoval.withDetails = function(data, Zcut, 
   ...
   adjDesign = NULL,
   pheno = NULL,
   blockSize = 5000, verbose = 0, indent = 0)
{
  spaces = indentSpaces(indent);
  if (verbose > 0) printFlush(spaste(spaces, "iterativeOutlierRemoval.withDetails..."));
  n = nrow(data);
  ...
  if (is.null(rowNames)) rowNames = spaste("Sample.", 1:nrow(data));
  while (!done)
  {
    if (verbose > 0 && scaleData) printFlush(paste(spaces, "..scaling data"));
    x = if (scaleData) scale(data[, useProbes]) else data[, useProbes];
    if (verbose > 0) printFlush(paste(spaces, "..calculating intersample connectivity..."));
    Z0 = intersampleDistanceConnectivity(x, blockSize = blockSize);
    Z0.scaled = if (robustScale) robustScale.1(Z0) else scale(Z0)
    if (length(adjDesign)>0 && adjDesign!="")
    {
      Zres = residuals(lm(as.formula(spaste("Z0", adjDesign)), data = as.data.frame(pheno)));
      Z.1 = if (robustScale) robustScale.1(Zres) else scale(Zres);
    } else Z.1 = Z0.scaled;
    outliers.1 = Z.1 > Zcut;
    if (verbose > 0) printFlush(paste(spaces, "..found", sum(outliers.1), "outliers"));
    tryCatch( { details = c(details, list(data.frame(sample = rownames(data), Z = Z.1, Z.unadjusted = Z0.scaled, outliers = outliers.1))) },
         error = function(e) {printFlush("iterativeOutlierRemoval.withdetails: caught error. Dropping into browser."); browser(e)});
    if (any(outliers.1))
    {
      outliers[keepIndex] = outliers.1;
      keepIndex = keepIndex[!outliers.1];
      data = data[!outliers.1, ];
      pheno = pheno[!outliers.1, ];
    } else done = TRUE
  }
