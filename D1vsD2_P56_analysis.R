# work directory
setwd("C:/Users/yanyanming77/Desktop/neuro_shape_analysis/D1_D2/D1vsD2_Oct-2024_clean")
...
############################ read data #######################################
###############################################################################################
data_dir = 'C:/Users/yanyanming77/Desktop/Lmeasure_extraction/D1_D2/Lmeasure_extract_all_features/May-20-2024/morpho_2248CPneurons_with_community.csv'
stats1 = read.csv(data_dir,check.names = FALSE)
colnames(stats1)
dim(stats1) # 2248

table(stats1$Type) # 1223 D1, 1025 D2
table(stats1$Striatal.Subregion) # 337 CPr, 1137 CPi, 774 CPc
...
# # Plot: for paper, Distribution plot of D1 and D2 in each brain, including all CP and non-CP neurons (2466)
# df_all = read.csv('C:/Users/yanyanming77/Desktop/Lmeasure_extraction/D1_D2/Lmeasure_extract_all_features/May-20-2024/morpho_2466CPneurons_with_community.csv')
# df_summary = df_all %>%
#   group_by(Brain, Type) %>%
...
#   left_join(df_summary, by = c("Brain"))
# 
# pdf(paste0("Plots/Type_Distribution_Brain_all_2466_neurons.pdf"), wi = 7, he = 6)
# p = ggplot(df_summary, aes(x = Brain, y = Count, fill = Type, label = Count)) +
...
draw_heatmap = function(data_for_heatmap, data_subset, group_name, only_profound = FALSE) {
  ...
  if(only_profound){
    feature_reorder = feature_reorder[feature_reorder$Feature %in% profound_featurs, ]
  }
...
pdf(file = spaste("Plots/13_Heatmap-Association-Type-Blue-Red-mean_diff", 'All_subregion', ".pdf"), wi = 5, he = 8);
scpp(2.5);
par(mar = c(3,5,3,5));
...
pdf(file = spaste("Plots/14_Heatmap-Association-Type-Blue-Red-mean_diff", 'All_subregion_community', ".pdf"), wi = 15, he = 8.5);
scpp(2.5);
par(mar = c(3,5,3,5));
...
pdf(file = spaste("Plots/15_Heatmap-Association-Type-Blue-Red-pct_change", 'All_subregion_community', ".pdf"), wi = 15, he = 8.5);
scpp(2.5);
par(mar = c(3,5,3,5));
  cat_features = c('Type', 'Type_in_CPr', 'Type_in_CPi', 'Type_in_CPc', c(paste0('Type_in_', order_community)))
